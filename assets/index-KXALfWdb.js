import"./jspdf-Bo0itkfF.js";import{r as Ye,u as je}from"./xlsx-DFH0qU2H.js";import{r as qe,a as Ze,g as Je}from"./mammoth-D8566hzF.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&o(r)}).observe(document,{childList:!0,subtree:!0});function n(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(a){if(a.ep)return;a.ep=!0;const i=n(a);fetch(a.href,i)}})();const G="CORALGENZ::MILSPEC_V6::QUANTUM_RESISTANT::ZERO_KNOWLEDGE::883920194821",Xe=G,Qe="CORALGENZ::MILSPEC_V5::ANTI_OFFLINE_CRACKER::ZERO_KNOWLEDGE::774910283419",et="CORALGENZ::MILSPEC_V4::ANTI_JOHN_THE_RIPPER::ZERO_KNOWLEDGE::992174829104";class h{constructor(){this.algo={name:"AES-GCM",length:256}}static detectDeviceCapabilities(){let t=4,n=4;try{typeof navigator<"u"&&(t=navigator.hardwareConcurrency||4,n=navigator.deviceMemory||4)}catch{}const o=t<=2||n<=2;return{isLowEnd:o,recommendedIterations:o?1e6:2e6,concurrency:t,memory:n}}static async computePayloadHash(t){const n=await window.crypto.subtle.digest("SHA-256",t);return Array.from(new Uint8Array(n)).map(a=>a.toString(16).padStart(2,"0")).join("")}static generateIV(){return window.crypto.getRandomValues(new Uint8Array(12))}static generateSalt(){return window.crypto.getRandomValues(new Uint8Array(32))}static async generateKey(){return window.crypto.subtle.generateKey({name:"AES-GCM",length:256},!0,["encrypt","decrypt","wrapKey","unwrapKey"])}static async deriveKeyFromPassword(t,n,o=2e6,a="SHA-256",i=!0,r=G){const s=new TextEncoder;let c;if(i){const u=r||Xe,y=s.encode(u),g=s.encode(t),f=new Uint8Array(y.length+g.length);f.set(y,0),f.set(g,y.length);const w=await window.crypto.subtle.importKey("raw",n,{name:"HMAC",hash:"SHA-512"},!1,["sign"]),p=await window.crypto.subtle.sign("HMAC",w,f);c=new Uint8Array(p),f.fill(0),g.fill(0)}else c=s.encode(t);const l=await window.crypto.subtle.importKey("raw",c,"PBKDF2",!1,["deriveKey"]),d=await window.crypto.subtle.deriveKey({name:"PBKDF2",salt:n,iterations:o,hash:a},l,{name:"AES-GCM",length:256},!0,["encrypt","decrypt","wrapKey","unwrapKey"]);return c&&c.fill&&c.fill(0),d}static async deriveKeyAsyncWorker(t,n,o=2e6,a=G){if(typeof Worker<"u"&&typeof Blob<"u"&&typeof URL<"u"&&URL.createObjectURL)try{return await new Promise((i,r)=>{const s=`
                    ...
                    `,c=new Blob([s],{type:"application/javascript"}),l=URL.createObjectURL(c),d=new Worker(l),u=setTimeout(()=>{try{d.terminate(),URL.revokeObjectURL(l)}catch{}r(new Error("Worker key derivation timed out"))},12e3);d.onmessage=async g=>{if(clearTimeout(u),d.terminate(),URL.revokeObjectURL(l),g.data&&g.data.success)try{const f=await window.crypto.subtle.importKey("raw",g.data.rawKey,{name:"AES-GCM",length:256},!0,["encrypt","decrypt","wrapKey","unwrapKey"]);i(f)}catch(f){r(f)}else r(new Error(g.data?.error||"Worker derivation failed"))},d.onerror=g=>{clearTimeout(u),d.terminate(),URL.revokeObjectURL(l),r(g)};const y=new Uint8Array(n);d.postMessage({password:t,salt:y,iterations:o,pepper:a})})}catch{}return this.deriveKeyFromPassword(t,n,o,"SHA-256",!0,a)}static async encryptData(t,n){const o=this.generateIV(),a=await window.crypto.subtle.encrypt({name:"AES-GCM",iv:o},t,n);return{iv:o,ciphertext:a}}static async decryptData(t,n,o){return window.crypto.subtle.decrypt({name:"AES-GCM",iv:n},t,o)}static async exportKey(t){return window.crypto.subtle.exportKey("raw",t)}static async importKey(t){return window.crypto.subtle.importKey("raw",t,{name:"AES-GCM"},!0,["encrypt","decrypt","wrapKey","unwrapKey"])}static async wrapKey(t,n){const o=this.generateIV(),a=await window.crypto.subtle.wrapKey("raw",t,n,{name:"AES-GCM",iv:o});return{iv:o,wrappedData:a}}static async unwrapKey(t,n,o){return window.crypto.subtle.unwrapKey("raw",t,n,{name:"AES-GCM",iv:o},{name:"AES-GCM"},!0,["encrypt","decrypt"])}static async unwrapWithFallback(t,n,o,a,i=2e6){try{const r=await this.deriveKeyFromPassword(n,o,i,"SHA-256",!0,G);return await this.unwrapKey(t,r,a)}catch(r){try{const s=await this.deriveKeyFromPassword(n,o,i,"SHA-256",!0,Qe);return await this.unwrapKey(t,s,a)}catch{try{const c=await this.deriveKeyFromPassword(n,o,1e6,"SHA-256",!0,et);return await this.unwrapKey(t,c,a)}catch{const l=[i,1e6,6e5,1e5];for(const d of l)try{const u=await this.deriveKeyFromPassword(n,o,d,"SHA-256",!1);return await this.unwrapKey(t,u,a)}catch{}throw r}}}}static async getDeviceKey(){const t=localStorage.getItem("sv_device_key");let n;if(t){const o=atob(t),a=new Uint8Array(o.length);for(let i=0;i<o.length;i++)a[i]=o.charCodeAt(i);n=a.buffer}else{const o=await this.generateKey();n=await this.exportKey(o);const a=new Uint8Array(n);let i="";for(let r=0;r<a.byteLength;r++)i+=String.fromCharCode(a[r]);localStorage.setItem("sv_device_key",btoa(i))}return this.importKey(n)}}const tt="SecureVaultDB",nt=1,P="files";function te(){return new Promise((e,t)=>{const n=indexedDB.open(tt,nt);n.onupgradeneeded=o=>{const a=o.target.result;a.objectStoreNames.contains(P)||a.createObjectStore(P,{keyPath:"id"})},n.onsuccess=o=>e(o.target.result),n.onerror=o=>t(o.target.error)})}const I={async saveFile(e){const t=await te();return new Promise((n,o)=>{const r=t.transaction(P,"readwrite").objectStore(P).put(e);r.onsuccess=()=>n(e.id),r.onerror=()=>o(r.error)})},async getAllFiles(){const e=await te();return new Promise((t,n)=>{const i=e.transaction(P,"readonly").objectStore(P).openCursor(),r=[];i.onsuccess=s=>{const c=s.target.result;if(c){const{content:l,...d}=c.value;r.push(d),c.continue()}else t(r)},i.onerror=()=>n(i.error)})},async getFile(e){const t=await te();return new Promise((n,o)=>{const r=t.transaction(P,"readonly").objectStore(P).get(e);r.onsuccess=()=>n(r.result),r.onerror=()=>o(r.error)})},async deleteFile(e){const t=await te();return new Promise((n,o)=>{const r=t.transaction(P,"readwrite").objectStore(P).delete(e);r.onsuccess=()=>n(),r.onerror=()=>o(r.error)})},async updateFile(e){return this.saveFile(e)}};var ot=qe(),rt=Ze();const at=Je(rt),se=document.getElementById("file-list");document.getElementById("add-file-btn");const Y=document.getElementById("auth-modal"),Re=document.getElementById("viewer"),_=document.getElementById("viewer-download-btn"),R=document.getElementById("file-input");document.getElementById("privacy-curtain");const it=document.getElementById("theme-toggle"),W=document.getElementById("search-input"),Oe=document.getElementById("sort-select"),st=document.getElementById("storage-text"),ct=document.getElementById("storage-fill"),Le=document.getElementById("bulk-actions"),lt=document.getElementById("selected-count"),dt=document.getElementById("bulk-delete-btn"),pt=document.getElementById("cancel-select-btn"),we=document.getElementById("info-modal"),ie=document.getElementById("rename-modal"),K=document.getElementById("strength-bar"),T=document.getElementById("strength-text"),ut=document.getElementById("bulk-export-btn"),ce=document.getElementById("recent-section"),le=document.getElementById("recent-scroll"),ne=document.getElementById("drop-zone"),Be=document.getElementById("stat-total"),Ce=document.getElementById("stat-images"),Te=document.getElementById("stat-videos"),Se=document.getElementById("stat-size"),U=document.getElementById("settings-modal"),Ke=document.getElementById("change-pass-modal"),mt=document.getElementById("settings-btn"),yt=document.getElementById("help-btn"),re=document.getElementById("help-modal");let he=null,D=null,O=[],L=new Set,oe=null,F=JSON.parse(localStorage.getItem("sv_recent")||"[]"),gt=null,pe=null,M={},N=null;function j(e,t="",n={}){return new Promise(o=>{const a=document.getElementById("custom-prompt-modal"),i=document.getElementById("prompt-title"),r=document.getElementById("prompt-message"),s=document.getElementById("prompt-input"),c=document.getElementById("prompt-confirm"),l=document.getElementById("prompt-cancel");i.textContent=e,r.textContent=t,s.type=n.inputType||"text",s.placeholder=n.placeholder||"Enter value...",s.value="";const d=()=>{a.close(),c.onclick=null,l.onclick=null,s.onkeydown=null};c.onclick=()=>{const u=s.value;d(),o(u||null)},l.onclick=()=>{d(),o(null)},s.onkeydown=u=>{u.key==="Enter"&&(u.preventDefault(),c.click())},a.showModal(),setTimeout(()=>s.focus(),100)})}function q(e,t=""){return new Promise(n=>{const o=document.getElementById("custom-confirm-modal"),a=document.getElementById("confirm-title"),i=document.getElementById("confirm-message"),r=document.getElementById("confirm-ok"),s=document.getElementById("confirm-cancel");a.textContent=e,i.textContent=t;let c=!1;const l=()=>{o.close(),r.onclick=null,s.onclick=null,o.removeEventListener("click",d),o.removeEventListener("close",u)},d=y=>{y.target===o&&!c&&(c=!0,l(),n(!1))},u=()=>{c||(c=!0,l(),n(!1))};r.onclick=()=>{c||(c=!0,l(),n(!0))},s.onclick=()=>{c||(c=!0,l(),n(!1))},o.addEventListener("click",d),o.addEventListener("close",u),o.showModal()})}function v(e,t=""){return new Promise(n=>{const o=document.getElementById("custom-alert-modal"),a=document.getElementById("alert-title"),i=document.getElementById("alert-message"),r=document.getElementById("alert-ok");a.textContent=e,i.textContent=t;const s=l=>{l.key==="Enter"&&(l.preventDefault(),c(),n())},c=()=>{o.close(),r.onclick=null,document.removeEventListener("keydown",s)};r.onclick=()=>{c(),n()},document.addEventListener("keydown",s),o.showModal()})}async function ft(){localStorage.removeItem("sv_app_lock"),localStorage.removeItem("sv_autolock"),localStorage.removeItem("sv_recovery_q"),localStorage.removeItem("sv_recovery_a"),It(),ge(),await S(),ht(),Jt(),ze(),be(),Xt()}ft();function b(e){const t=document.getElementById("security-terminal-body");if(t){const n=document.createElement("p");n.className="term-line",n.textContent=`> ${e}`,t.appendChild(n),t.scrollTop=t.scrollHeight}}function k(e,t="completed"){const n=document.getElementById(e);if(n){n.className=`process-step ${t}`.trim();const o=n.querySelector(".step-icon");o&&(t==="completed"?o.textContent="[✓]":t==="active"?o.textContent="[→]":o.textContent="[ ]")}}function H(e,t){const n=document.getElementById("protect-progress-bar"),o=document.getElementById("protect-progress-percent"),a=document.getElementById("protect-progress-label");n&&(n.style.width=`${e}%`),o&&(o.textContent=`${e}%`),a&&t&&(a.textContent=t)}function wt(){const e="ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#$%^&*()_+",t=new Uint8Array(24);window.crypto.getRandomValues(t);let n="";for(let a=0;a<24;a++)n+=e[t[a]%e.length];const o=document.getElementById("new-password");if(o){o.value=n,o.type="text",_e();const a=document.getElementById("toggle-new-password");a&&(a.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="23" x2="23" y2="1"></line></svg>',a.style.color="var(--cyber-cyan)")}}function ht(){document.getElementById("cancel-add").addEventListener("click",()=>{z()}),document.getElementById("confirm-add").addEventListener("click",De),document.getElementById("generate-pwd-btn")?.addEventListener("click",wt);const e=document.getElementById("toggle-new-password"),t=document.getElementById("new-password");e?.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),t&&(t.type==="password"?(t.type="text",e.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="23" x2="23" y2="1"></line></svg>',e.style.color="var(--cyber-cyan)"):(t.type="password",e.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>',e.style.color="var(--cyber-text-muted)"))}),t?.addEventListener("keydown",r=>{r.key==="Enter"&&(r.preventDefault(),De())}),document.getElementById("download-protected-html-btn")?.addEventListener("click",async()=>{if(!N||!N.record){v("Notice","No active protected file session. Please select a file from the vault below to download.");return}const r=document.getElementById("download-protected-html-btn"),s=r?r.innerHTML:"";try{r&&(r.disabled=!0,r.innerHTML=`
        ...
        `);let c=N.buffer,l=N.password;if(!c||c.byteLength===0){const d=await X(N.record,l);if(d&&d.buffer)c=d.buffer,l=d.password||l;else throw new Error("Could not retrieve file content for packaging.")}await J(N.record,c,l,{},"Protected HTML package exported successfully!")}catch(c){console.error(c),await v("Export Error","Failed to download HTML package: "+c.message)}finally{r&&(r.disabled=!1,r.innerHTML=s)}}),document.getElementById("view-in-vault-btn")?.addEventListener("click",()=>{z(),document.getElementById("secured-files-heading")?.scrollIntoView({behavior:"smooth"})}),document.getElementById("protect-another-btn")?.addEventListener("click",()=>{z(),document.getElementById("inline-add-container")?.scrollIntoView({behavior:"smooth"})}),document.querySelectorAll(".cyber-nav-item").forEach(r=>{r.addEventListener("click",()=>{document.querySelectorAll(".cyber-nav-item").forEach(s=>s.classList.remove("active")),r.classList.add("active"),r.id==="nav-item-protect"&&z()})}),document.getElementById("sidebar-settings-btn")?.addEventListener("click",()=>{ge(),U?.showModal()}),document.getElementById("cancel-auth").addEventListener("click",()=>{Y.close(),document.getElementById("auth-password").value="",he=null}),document.getElementById("confirm-auth").addEventListener("click",Ne),document.getElementById("auth-password")?.addEventListener("keydown",r=>{r.key==="Enter"&&(r.preventDefault(),Ne())}),document.getElementById("close-viewer").addEventListener("click",ye),it?.addEventListener("click",He),W?.addEventListener("input",Lt),Oe?.addEventListener("change",Bt),document.getElementById("new-password")?.addEventListener("input",_e),dt?.addEventListener("click",At),ut?.addEventListener("click",Pt),pt?.addEventListener("click",St),document.getElementById("select-all-btn")?.addEventListener("click",Tt),document.getElementById("close-info")?.addEventListener("click",()=>we.close()),document.getElementById("cancel-rename")?.addEventListener("click",()=>ie.close()),document.getElementById("confirm-rename")?.addEventListener("click",Pe),document.getElementById("rename-input")?.addEventListener("keydown",r=>{r.key==="Enter"&&(r.preventDefault(),Pe())}),Dt(),Rt(),yt?.addEventListener("click",()=>re?.showModal()),document.getElementById("close-help")?.addEventListener("click",()=>re?.close()),kt(),mt?.addEventListener("click",()=>{ge(),U?.showModal()}),document.getElementById("close-settings")?.addEventListener("click",()=>U?.close()),document.getElementById("close-settings-x")?.addEventListener("click",()=>U?.close()),document.getElementById("feedback-btn")?.addEventListener("click",()=>{const r="coralgenz@zohomail.in",s=encodeURIComponent("SecureVault Feedback"),c=encodeURIComponent(`Hi team,

I have some feedback for SecureVault:
`);window.location.href=`mailto:${r}?subject=${s}&body=${c}`}),document.querySelectorAll(".panic-trigger").forEach(r=>{r.addEventListener("click",fe)}),document.getElementById("panic-btn")?.addEventListener("click",fe),document.getElementById("panic-action-select")?.addEventListener("change",r=>{localStorage.setItem("sv_panic_action",r.target.value)}),document.getElementById("panic-enable-toggle")?.addEventListener("change",r=>{const s=r.target.checked;localStorage.setItem("sv_panic_enabled",s),be()}),document.getElementById("dl-media-toggle")?.addEventListener("change",r=>{localStorage.setItem("sv_dl_media",r.target.checked),ue()}),document.getElementById("dl-doc-toggle")?.addEventListener("change",r=>{localStorage.setItem("sv_dl_doc",r.target.checked),ue()}),document.getElementById("cancel-change-pass")?.addEventListener("click",()=>Ke?.close()),document.getElementById("confirm-change-pass")?.addEventListener("click",Ht),de("toggle-new-password","new-password"),de("toggle-auth-password","auth-password"),de("toggle-share-password","share-password");const n=document.getElementById("share-modal");document.getElementById("cancel-share")?.addEventListener("click",()=>n?.close()),document.getElementById("confirm-share")?.addEventListener("click",Me),document.getElementById("share-password")?.addEventListener("keydown",r=>{r.key==="Enter"&&(r.preventDefault(),Me())}),document.getElementById("share-logo-zone")?.addEventListener("click",r=>{r.target!==document.getElementById("share-logo")&&document.getElementById("share-logo")?.click()}),document.getElementById("share-logo")?.addEventListener("change",r=>{const s=r.target.files[0],c=document.getElementById("share-logo-text");c&&(c.textContent=s?s.name:"Choose File")}),R?.addEventListener("change",Fe),document.getElementById("file-remove-btn")?.addEventListener("click",vt);const o=document.getElementById("file-upload-zone"),a=document.getElementById("drop-text-primary"),i=document.getElementById("drop-text-secondary");o?.addEventListener("click",r=>{r.target!==R&&R?.click()}),o?.addEventListener("dragover",r=>{r.preventDefault(),o.classList.add("drag-over"),a&&(a.textContent="FILE DETECTED — INITIALIZING PROTECTION"),i&&(i.textContent="Release file to initialize security inspection")}),o?.addEventListener("dragleave",()=>{o.classList.remove("drag-over"),a&&(a.textContent="DROP FILE TO SECURE"),i&&(i.textContent="or click to browse local file system")}),o?.addEventListener("drop",()=>{o.classList.remove("drag-over"),a&&(a.textContent="DROP FILE TO SECURE"),i&&(i.textContent="or click to browse local file system")}),document.addEventListener("contextmenu",r=>r.preventDefault()),document.addEventListener("keydown",r=>{(r.key==="PrintScreen"||r.ctrlKey&&r.key==="p")&&(r.preventDefault(),alert("Screenshots are disabled"))})}function de(e,t){const n=document.getElementById(e),o=document.getElementById(t);!n||!o||n.addEventListener("click",a=>{a.preventDefault(),o.type==="password"?(o.type="text",n.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="23" x2="23" y2="1"></line></svg>',n.style.color="var(--cyber-cyan)"):(o.type="password",n.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>',n.style.color="var(--cyber-muted)")})}function z(){R.value="",document.getElementById("new-password").value="",N=null,K.className="strength-bar",T.innerText="ENTER PASSWORD";const e=document.getElementById("file-upload-zone"),t=document.getElementById("file-preview"),n=document.getElementById("inline-password-section"),o=document.getElementById("protection-process-container"),a=document.getElementById("protection-complete-container");e?.classList.remove("hidden"),t?.classList.add("hidden"),n?.classList.add("hidden"),o?.classList.add("hidden"),a?.classList.add("hidden");const i=document.getElementById("process-timer-display");i&&(i.textContent="00:00.00");const r=document.querySelector(".scanner-center-shield");r&&r.classList.remove("success");const s=document.getElementById("scanner-status-text");s&&(s.textContent="SECURING");const c=document.getElementById("security-terminal-body");c&&(c.innerHTML='<p class="term-line">&gt; Standby for cryptographic instruction...</p>'),["step-analysis","step-prep","step-kdf","step-encrypt","step-meta","step-finalize","step-output","step-verify"].forEach(l=>{k(l,"")}),H(0,"STANDBY")}function Fe(){const e=R.files[0];if(!e)return;const t=150*1024*1024;if(e.size>t){v("File Too Large","Please select a file smaller than 150 MB."),R.value="";return}const n=document.getElementById("file-upload-zone"),o=document.getElementById("file-preview"),a=document.getElementById("file-preview-name"),i=document.getElementById("file-preview-size"),r=document.getElementById("protection-process-container"),s=document.getElementById("protection-complete-container");a&&(a.textContent=e.name),i&&(i.textContent=ae(e.size)),n?.classList.add("hidden"),r?.classList.add("hidden"),s?.classList.add("hidden"),o?.classList.remove("hidden"),document.getElementById("inline-password-section")?.classList.remove("hidden")}function vt(e){e.preventDefault(),e.stopPropagation(),R.value="",N=null;const t=document.getElementById("file-upload-zone"),n=document.getElementById("file-preview"),o=document.getElementById("inline-password-section"),a=document.getElementById("protection-process-container"),i=document.getElementById("protection-complete-container");t?.classList.remove("hidden"),n?.classList.add("hidden"),o?.classList.add("hidden"),a?.classList.add("hidden"),i?.classList.add("hidden"),t?.classList.remove("hidden"),n?.classList.add("hidden"),document.getElementById("inline-password-section")?.classList.add("hidden")}function ae(e){if(e===0)return"0 Bytes";const t=1024,n=["Bytes","KB","MB","GB"],o=Math.floor(Math.log(e)/Math.log(t));return parseFloat((e/Math.pow(t,o)).toFixed(2))+" "+n[o]}function bt(e,t){try{const n=Ye(new Uint8Array(e),{type:"array"}),o=n.SheetNames;if(o.length===0)return;const a=document.createElement("div");a.className="excel-viewer";const i=document.createElement("div");i.className="excel-header";const r=document.createElement("div");r.className="excel-table-wrapper";let s=o[0];const c=l=>{r.innerHTML="";const d=n.Sheets[l],u=je.sheet_to_json(d,{header:1,defval:""});if(!u||u.length===0){r.innerHTML='<div style="padding:20px;text-align:center;">Empty Sheet</div>';return}const y=document.createElement("table");y.className="excel-table",u.forEach((g,f)=>{const w=document.createElement("tr");g.forEach(p=>{const m=f===0?"th":"td",E=document.createElement(m);E.textContent=p!==void 0?p:"",w.appendChild(E)}),y.appendChild(w)}),r.appendChild(y)};o.forEach(l=>{const d=document.createElement("button");d.className=`excel-sheet-btn ${l===s?"active":""}`,d.textContent=l,d.onclick=()=>{s=l,c(l),i.querySelectorAll(".excel-sheet-btn").forEach(u=>u.classList.remove("active")),d.classList.add("active")},i.appendChild(d)}),c(s),a.appendChild(i),a.appendChild(r),t.appendChild(a)}catch(n){console.error(n),t.innerHTML=`<div style="padding:20px;color:red;">Error parsing Excel file: ${n.message}</div>`}}async function Et(e,t){try{const{value:n,messages:o}=await ot.convertToHtml({arrayBuffer:e}),a=document.createElement("div");a.className="word-viewer";const i=document.createElement("div");i.className="word-document",i.innerHTML=n,a.appendChild(i),t.appendChild(a)}catch(n){console.error(n),t.innerHTML=`<div style="padding:20px;color:red;">Error parsing Word file: ${n.message}</div>`}}async function xt(e,t,n){try{const o=await at.loadAsync(e),a=Object.keys(o.files).filter(u=>u.startsWith("ppt/slides/slide")&&u.endsWith(".xml"));if(a.sort((u,y)=>{const g=parseInt(u.replace(/[^0-9]/g,"")||"0",10),f=parseInt(y.replace(/[^0-9]/g,"")||"0",10);return g-f}),a.length===0)throw new Error("No slide XML found in presentation archive.");const i=[],r=new DOMParser;for(const u of a){const y=await o.files[u].async("string"),f=r.parseFromString(y,"application/xml").querySelectorAll("t"),w=Array.from(f).map(p=>p.textContent.trim()).filter(Boolean);i.push({title:w[0]||`Slide ${i.length+1}`,body:w.slice(1).join(`
`)||"Slide Content"})}let s=0;const c=document.createElement("div");c.className="ppt-viewer";const l=document.createElement("div");l.className="ppt-slide-card";const d=u=>{const y=i[u];l.innerHTML=`
        ...
        <div class="ppt-nav-bar">
          ...
          <span style="font-family:var(--font-mono);font-size:12px;font-weight:700;color:var(--accent-cyan);">Slide ${u+1} of ${i.length}</span>
          <button type="button" id="ppt-next-btn" class="excel-sheet-btn" ${u===i.length-1?'disabled style="opacity:0.5;"':""}>Next ▶</button>
        </div>
      `,l.querySelector("#ppt-prev-btn")?.addEventListener("click",()=>{s>0&&(s--,d(s))}),l.querySelector("#ppt-next-btn")?.addEventListener("click",()=>{s<i.length-1&&(s++,d(s))})};d(0),c.appendChild(l),n.appendChild(c)}catch(o){console.warn("PPTX parsing fallback:",o),n.innerHTML=`
    ...
    `}}function kt(){document.querySelectorAll("dialog.modal").forEach(t=>{t.addEventListener("click",n=>{const o=t.getBoundingClientRect();n.clientX>=o.left&&n.clientX<=o.right&&n.clientY>=o.top&&n.clientY<=o.bottom,n.target===t&&(t.close(),t.id==="add-modal"&&z())})})}function It(){localStorage.getItem("sv_theme")==="light"&&document.body.classList.add("light-theme")}function He(){document.body.classList.toggle("light-theme");const e=document.body.classList.contains("light-theme");localStorage.setItem("sv_theme",e?"light":"dark")}function _e(){const e=document.getElementById("new-password")?.value||"",n=["123456","password","12345678","admin","admin123","benz","qwerty","123456789","welcome","letmein","monkey","dragon","master","football","access","iloveyou","testing","security","vault","pass123"].includes(e.toLowerCase().trim());let o=0;e.length>=8&&o++,e.length>=12&&o++,e.length>=18&&o++,/[A-Z]/.test(e)&&/[a-z]/.test(e)&&o++,/[0-9]/.test(e)&&o++,/[^A-Za-z0-9]/.test(e)&&o++,n&&(o=1),K&&T&&(K.className="strength-bar",e?o<=2||n?(K.classList.add("weak"),T.innerText=n?"VULNERABLE (DICTIONARY WORD)":"WEAK (GPU CRACKABLE)",T.style.color="#ef4444"):o===3||o===4?(K.classList.add("fair"),T.innerText="MODERATE (RECOMMEND 12+ CHARS)",T.style.color="#f59e0b"):o===5?(K.classList.add("good"),T.innerText="STRONG (GPU RESISTANT)",T.style.color="#0284c7"):(K.classList.add("strong"),T.innerText="MIL-SPEC // QUANTUM RESISTANT",T.style.color="#10b981"):(T.innerText="ENTER PASSWORD",T.style.color="var(--cyber-text-muted)",K.style.width="0%"))}function Lt(){const e=W.value.toLowerCase().trim();S(e)}function Bt(){S(W?.value||"")}function Ct(e){const t=e.reduce((r,s)=>r+(s.size||0),0),n=(t/1024/1024).toFixed(2);st.innerText=`${e.length} files • ${n} MB used`;const o=Math.min(t/(500*1024*1024)*100,100);ct.style.width=o+"%";const a=e.filter(r=>r.type?.startsWith("image")).length,i=e.filter(r=>r.type?.startsWith("video")).length;Be&&(Be.innerText=e.length),Ce&&(Ce.innerText=a),Te&&(Te.innerText=i),Se&&(Se.innerText=n)}function Z(){const e=document.getElementById("select-all-btn");L.size>0?(Le.classList.remove("hidden"),lt.innerText=`${L.size} selected`,e&&(L.size>=O.length&&O.length>0?e.innerText="Deselect All":e.innerText="Select All")):Le.classList.add("hidden")}function Tt(){L.size>=O.length&&O.length>0?L.clear():O.forEach(e=>L.add(e.id)),Z(),S(W?.value||"")}function St(){L.clear(),Z(),S(W?.value||"")}async function At(){if(await q("Delete Files",`Delete ${L.size} file(s)? This cannot be undone.`)){for(const t of L)await I.deleteFile(t);L.clear(),Z(),S(),await v("Success","Files deleted.")}}async function Pt(){if(L.size!==0){await v("Export",`Exporting ${L.size} files. Each will download separately.`);for(const e of L)await I.getFile(e)&&await Mt(e);L.clear(),Z(),S()}}async function Mt(e){const t=await I.getFile(e);if(!t)return;const n=await X(t);if(!n||!n.buffer)return;const{buffer:o,password:a}=n;let i=a;i||(i=await j("Export Protected File","Enter the file's password to protect this export:",{inputType:"password",placeholder:"Enter original password..."})),i&&await J(t,o,i,{},"Protected file downloaded successfully!")}function Dt(){const e=document.getElementById("app");["dragenter","dragover"].forEach(t=>{e?.addEventListener(t,n=>{n.preventDefault(),ne?.classList.remove("hidden"),ne?.classList.add("active")})}),["dragleave","drop"].forEach(t=>{e?.addEventListener(t,n=>{n.preventDefault(),ne?.classList.add("hidden"),ne?.classList.remove("active")})}),e?.addEventListener("drop",async t=>{t.preventDefault();const n=t.dataTransfer?.files;if(n&&n.length>0)if(n.length===1){const o=new DataTransfer;o.items.add(n[0]),R.files=o.files,Fe(),document.getElementById("inline-add-container")?.scrollIntoView({behavior:"smooth"})}else for(const o of n)await Nt(o)})}async function Nt(e){if(e.size>157286400){await v("File Too Large",`File "${e.name}" is too large. Max 150 MB.`);return}const n=await j("Set Password",`Set password for: ${e.name}`,{inputType:"password",placeholder:"Enter secure password..."});if(!n)return;const o=await h.generateKey(),a=h.generateSalt(),i=await h.deriveKeyFromPassword(n,a,2e6),r=await e.arrayBuffer(),{iv:s,ciphertext:c}=await h.encryptData(o,r),{iv:l,wrappedData:d}=await h.wrapKey(o,i),u={id:crypto.randomUUID(),name:e.name,type:e.type,size:e.size,date:Date.now(),authMode:"always",keys:[{type:"password",salt:a,iv:l,data:d}],content:c,iv:s,viewCount:0};await I.saveFile(u),S(),await v("Success",`${e.name} encrypted and saved!`)}function Rt(){document.addEventListener("keydown",e=>{(e.ctrlKey||e.metaKey)&&e.key==="n"&&e.preventDefault(),(e.ctrlKey||e.metaKey)&&e.key==="f"&&(e.preventDefault(),W?.focus()),e.key==="Escape"&&(z(),Y.close(),we?.close(),ie?.close()),(e.ctrlKey||e.metaKey)&&e.shiftKey&&e.key.toLowerCase()==="p"&&(e.preventDefault(),fe()),(e.ctrlKey||e.metaKey)&&e.key==="t"&&(e.preventDefault(),He())})}const Ae=5,Ot=300*1e3;function Ue(e){const t=M[e];if(t&&t.locked&&Date.now()<t.lockedUntil){const n=Math.ceil((t.lockedUntil-Date.now())/1e3);return alert(`File locked. Try again in ${n} seconds.`),!0}return!1}function Kt(e){M[e]||(M[e]={count:0,locked:!1,lockedUntil:0}),M[e].count++;const t=Ae-M[e].count,n=document.getElementById("attempts-left"),o=document.getElementById("auth-attempts");t<=3&&(o?.classList.remove("hidden"),n&&(n.innerText=t)),M[e].count>=Ae&&(M[e].locked=!0,M[e].lockedUntil=Date.now()+Ot,Y.close(),alert("Too many failed attempts. File locked for 5 minutes."))}function Ft(e){delete M[e],document.getElementById("auth-attempts")?.classList.add("hidden")}async function Ht(){const e=document.getElementById("current-password").value,t=document.getElementById("new-password-change").value,n=document.getElementById("confirm-password-change").value;if(!e||!t||!n){await v("Required","Please fill all fields");return}if(t!==n){await v("Error","New passwords do not match");return}const o=["123456","password","12345678","admin","admin123","benz","qwerty","123456789","welcome","letmein","monkey","dragon","master","football","access","iloveyou","testing","security","vault","pass123","root","user","111111","000000","trustno1"],a=t.toLowerCase().trim();if(!((t.length<8||o.includes(a))&&!await q("⚠️ Weak Password Warning",`This password is short or common, which makes it less secure against offline dictionary attacks. However, Coralgenz Vault still protects it with 2,000,000 PBKDF2 rounds.

Do you want to proceed with this password?`)))try{const r=await I.getFile(gt);if(!r)throw new Error("File not found");const s=r.keys.find(g=>g.type==="password");if(!s)throw new Error("No password key found");const c=await h.unwrapWithFallback(s.data,e,s.salt,s.iv),l=h.generateSalt(),d=await h.deriveKeyFromPassword(t,l,2e6),{iv:u,wrappedData:y}=await h.wrapKey(c,d);r.keys=r.keys.filter(g=>g.type!=="password"),r.keys.push({type:"password",salt:l,iv:u,data:y}),r.accessLog=r.accessLog||[],r.accessLog.push({action:"password_changed",date:Date.now()}),await I.updateFile(r),Ke?.close(),await v("Success","Password changed successfully!")}catch(r){console.error(r),await v("Error","Failed to change password. Current password may be incorrect.")}}function _t(e,t){e.accessLog=e.accessLog||[],e.accessLog.push({action:t,date:Date.now()}),e.accessLog.length>20&&(e.accessLog=e.accessLog.slice(-20))}function Ut(e){F=F.filter(t=>t.id!==e.id),F.unshift({id:e.id,name:e.name,type:e.type,date:Date.now()}),F.length>5&&(F=F.slice(0,5)),localStorage.setItem("sv_recent",JSON.stringify(F)),ze()}function ze(){if(!ce||!le)return;const e=F.filter(t=>O.some(n=>n.id===t.id));if(e.length===0){ce.classList.add("hidden");return}ce.classList.remove("hidden"),le.innerHTML=e.map(t=>`
    <div class="recent-item" data-id="${t.id}">
      <span>${t.type?.startsWith("image")?"🖼️":t.type?.startsWith("video")?"🎬":"📄"}</span>
      <span>${t.name}</span>
    </div>
  `).join(""),le.querySelectorAll(".recent-item").forEach(t=>{t.onclick=()=>me(t.dataset.id)})}async function zt(e,t){e.stopPropagation(),await q("Delete File","Delete this file permanently?")&&(await I.deleteFile(t),S())}async function Wt(e,t){e.stopPropagation();const n=O.find(i=>i.id===t);if(!n)return;document.getElementById("info-name").innerText=n.name,document.getElementById("info-type").innerText=n.type||"Unknown",document.getElementById("info-size").innerText=(n.size/1024/1024).toFixed(2)+" MB",document.getElementById("info-date").innerText=new Date(n.date).toLocaleDateString(),document.getElementById("info-mode").innerText=n.authMode==="always"?"Always Ask Password":n.authMode||"Always Ask Password";const o=document.getElementById("info-views");o&&(o.innerText=n.viewCount||0);const a=document.getElementById("access-log");a&&n.accessLog&&n.accessLog.length>0?a.innerHTML=n.accessLog.slice(-10).reverse().map(i=>`
      <div class="access-log-item">
        ${i.action.replace("_"," ")} - ${new Date(i.date).toLocaleString()}
      </div>
    `).join(""):a&&(a.innerHTML="No access history"),we.showModal()}function Vt(e,t,n){e.stopPropagation(),oe=t;const o=document.getElementById("rename-input");o.value=n,ie.showModal(),setTimeout(()=>{o.focus(),o.select()},100)}async function Pe(){const e=document.getElementById("rename-input").value.trim();if(!e||!oe)return;const t=await I.getFile(oe);t&&(t.name=e,await I.updateFile(t)),ie.close(),oe=null,S()}async function $t(e,t){e&&e.stopPropagation();try{const n=await I.getFile(t);if(!n){await v("Error","File not found");return}const o=await X(n);if(!o||!o.buffer)return;const{buffer:a,password:i}=o;let r=i;if(r||(r=await j("Download Protected File","Enter the file's password to protect this download:",{inputType:"password",placeholder:"Enter original password..."})),!r)return;await J(n,a,r,{},"Downloaded! The protected file has been saved."),_t(n,"downloaded"),await I.updateFile(n)}catch(n){console.error(n),await v("Error","Failed to download file: "+(n.message||"Decryption error"))}}async function Gt(e,t){e.stopPropagation();try{const n=await I.getFile(t);if(!n)return;const o=await X(n);if(!o||!o.buffer)return;const{buffer:a,password:i}=o;let r=i;if(r||(r=await j("Share File","Enter the file's password to protect this export:",{inputType:"password",placeholder:"Enter original password..."})),!r)return;await J(n,a,r,{})}catch(n){console.error(n),await v("Error","Share failed: "+n.message)}}function Yt(e,t){e.stopPropagation(),pe=t,document.getElementById("share-password").value="",document.getElementById("share-title").value="",document.getElementById("share-logo").value="",document.getElementById("share-modal")?.showModal()}async function Me(){const e=document.getElementById("share-modal"),t=document.getElementById("share-password").value,n=document.getElementById("share-title").value||"SecureVault",o=document.getElementById("share-logo");if(!t){await v("Required","Please set a password for the file.");return}const a=["123456","password","12345678","admin","admin123","benz","qwerty","123456789","welcome","letmein","monkey","dragon","master","football","access","iloveyou","testing","security","vault","pass123","root","user","111111","000000","trustno1"],i=t.toLowerCase().trim();if((t.length<8||a.includes(i))&&!await q("⚠️ Weak Password Warning",`This password is short or common, which makes it less secure against offline dictionary attacks. However, Coralgenz Vault still protects it with 2,000,000 PBKDF2 rounds.

Do you want to proceed with this password?`))return;const s=document.getElementById("confirm-share"),c=s.innerText;s.innerText="Exporting...";try{const l=await I.getFile(pe);if(!l)throw new Error("File not found");const d=await X(l,t);if(!d||!d.buffer)throw new Error("Decryption failed");const u=d.buffer;let y="";o.files&&o.files[0]&&(y=await new Promise(f=>{const w=new FileReader;w.onload=()=>f(w.result),w.readAsDataURL(o.files[0])})),await J(l,u,t,{title:n,logoUrl:y}),e.close(),pe=null}catch(l){console.error(l),await v("Error","Export failed: "+l.message)}finally{s.innerText=c}}function jt(e,t){const n=(t||"").toLowerCase(),o=(e||"").toLowerCase();return n.startsWith("image/")||n.startsWith("video/")||n.startsWith("audio/")?!0:/\.(jpg|jpeg|png|gif|webp|svg|bmp|ico|mp4|webm|mov|mkv|ogg|mp3|wav|aac|m4a|flac)$/i.test(o)}function We(){const e=localStorage.getItem("sv_dl_media")!=="false",t=localStorage.getItem("sv_dl_doc")!=="false";return{allowMedia:e,allowDoc:t}}function ve(e,t){const{allowMedia:n,allowDoc:o}=We();return jt(e,t)?n:o}function ue(){const{allowMedia:e,allowDoc:t}=We(),n=document.getElementById("dl-media-status-badge"),o=document.getElementById("dl-doc-status-badge");n&&(n.textContent=e?"ALLOWED":"RESTRICTED",n.className=`policy-status-badge ${e?"allowed":"restricted"}`),o&&(o.textContent=t?"ALLOWED":"RESTRICTED",o.className=`policy-status-badge ${t?"allowed":"restricted"}`)}async function J(e,t,n,o={},a="Protected file downloaded successfully!"){if(!t||t.byteLength===0)throw new Error("No decrypted file payload available to package.");if(!n)throw new Error("Protection password is required to export this file.");const i=o.allowDownload!==void 0?!!o.allowDownload:ve(e.name,e.type),r=h.generateSalt(),s=await h.computePayloadHash(t),c=await h.deriveKeyAsyncWorker(n,r,2e6,h.MILSPEC_ANTI_CRACKER_PEPPER_V6),{iv:l,ciphertext:d}=await h.encryptData(c,t),y=await(x=>new Promise(C=>{const A=new FileReader;A.readAsDataURL(x),A.onloadend=()=>{const V=A.result||"";C(V.split(",")[1]||"")}}))(new Blob([d]));if(!y)throw new Error("Failed to serialize encrypted payload.");const{header:g,footer:f}=qt(e,r,l,{...o,allowDownload:i,integrityHash:s}),w=new Blob([g,y,f],{type:"text/html;charset=utf-8"}),p=URL.createObjectURL(w),m=document.createElement("a");m.href=p;const E=e.name||"protected_file";m.download=E.endsWith(".secure.html")?E:E+".secure.html",document.body.appendChild(m),m.click(),document.body.removeChild(m),setTimeout(()=>URL.revokeObjectURL(p),6e4),a&&await v("Success",a)}function qt(e,t,n,o={}){const a=ee=>btoa(String.fromCharCode(...new Uint8Array(ee))),i=a(t),r=a(n),s=o.title||"Coralgenz Vault",c=o.logoUrl||"",l=e.name||"Protected File",d=e.type||"application/octet-stream",u=Number(e.size)||0,y=e.id||"",g=o.allowDownload!==void 0?!!o.allowDownload:ve(l,d),f=JSON.stringify(i),w=JSON.stringify(r),p=JSON.stringify(d),m=JSON.stringify(l),E=JSON.stringify(u),x=JSON.stringify(s),C=JSON.stringify(y),A=JSON.stringify(g),V=JSON.stringify(o.integrityHash||e.integrityHash||""),$=JSON.stringify("V6"),Q=c?`<img src="${c}" alt="Logo" class="brand-custom-logo">`:`<div class="brand-shield-icon">
...
<body>
        ...
        <div class="input-group">
            ...
            <div id="virtual-keypad-container" class="virtual-keypad-drawer hidden">
                <div class="keypad-header">
                    <span class="keypad-title">ORDERLY VIRTUAL KEYPAD // BLINDS OS KEYLOGGERS</span>
                    <button type="button" id="keypad-caps-btn" class="keypad-tool-btn" title="Toggle Caps Lock">⇪ CAPS: ON</button>
                </div>
                <div id="virtual-keypad-body"></div>
                <div class="keypad-actions" style="margin-top:8px;display:flex;gap:4px;">
                    <button type="button" class="keypad-key-btn control-key" id="keypad-space" style="flex:1;">⎵ SPACE</button>
    ...
    <script>
        ...
        // Anti-Keylogger Orderly Virtual Keypad Engine (No Scramble)
        const keypadContainer = document.getElementById('virtual-keypad-container');
        const keypadBody = document.getElementById('virtual-keypad-body');
        const toggleKeypadBtn = document.getElementById('toggle-keypad-btn');
        const capsBtn = document.getElementById('keypad-caps-btn');
        ...
        function renderVirtualKeypad() {
            if (!keypadBody) return;
            keypadBody.innerHTML = '';

            // Section 1: Numbers
            const numLabel = document.createElement('div');
            numLabel.className = 'keypad-title';
            numLabel.style.margin = '4px 0 2px';
            numLabel.textContent = 'NUMBERS (1 - 0)';
            keypadBody.appendChild(numLabel);

            const numRow = document.createElement('div');
            numRow.style.display = 'grid';
            numRow.style.gridTemplateColumns = 'repeat(10, 1fr)';
            numRow.style.gap = '3px';
            numRow.style.marginBottom = '6px';

            KEY_NUMBERS.forEach(ch => {
                ...
                btn.onclick = (e) => {
                    e.preventDefault();
                    if (pwdInput) pwdInput.value += ch;
                };
                numRow.appendChild(btn);
            });
            keypadBody.appendChild(numRow);

            // Section 2: Letters
            const letLabel = document.createElement('div');
            letLabel.className = 'keypad-title';
            letLabel.style.margin = '6px 0 2px';
            letLabel.textContent = 'LETTERS (A - Z)';
            keypadBody.appendChild(letLabel);

            KEY_QWERTY.forEach(rowArr => {
                ...
                rowArr.forEach(ch => {
                    const charToDisplay = isCaps ? ch : ch.toLowerCase();
                    const btn = document.createElement('button');
                    btn.type = 'button';
                    btn.className = 'keypad-key-btn';
                    btn.style.flex = '1';
                    btn.style.minWidth = '0';
                    btn.textContent = charToDisplay;
                    btn.onclick = (e) => {
                        e.preventDefault();
                        if (pwdInput) pwdInput.value += charToDisplay;
                    };
                    row.appendChild(btn);
                });
                keypadBody.appendChild(row);
            });

            // Section 3: Symbols
            const symLabel = document.createElement('div');
            symLabel.className = 'keypad-title';
            symLabel.style.margin = '6px 0 2px';
            symLabel.textContent = 'SPECIAL CHARACTERS';
            keypadBody.appendChild(symLabel);

            KEY_SYMBOLS.forEach(rowArr => {
                const row = document.createElement('div');
                row.style.display = 'flex';
                row.style.gap = '3px';
                row.style.marginBottom = '3px';
        ...
        capsBtn?.addEventListener('click', (e) => {
            e.preventDefault();
            isCaps = !isCaps;
            capsBtn.textContent = 'CAPS: ' + (isCaps ? 'ON' : 'OFF');
            renderVirtualKeypad();
        });
        ...
        document.getElementById('keypad-space')?.addEventListener('click', (e) => {
            e.preventDefault();
        ...
        async function unlock() {
            ...
            try {
                ...
                } else if (/.(csv|tsv)$/i.test(lowerName)) {
...
/).filter(l => l.trim().length > 0);
                ...
                } else if (/.(xlsx|xls)$/i.test(lowerName) || safeType.includes('excel') || safeType.includes('spreadsheet')) {
                ...
                } else if (/.(docx|doc)$/i.test(lowerName) || safeType.includes('word')) {
                    const card = document.createElement('div');
                    card.className = 'fallback-download-card';
                ...
                } else if (/.(pptx|ppt|pps|ppsx)$/i.test(lowerName) || safeType.includes('presentation') || safeType.includes('powerpoint')) {
                    const card = document.createElement('div');
                    card.className = 'fallback-download-card';
...
</html>`}}async function X(e,t=null){let n=null,o=t;if(t)try{const i=e.keys.find(r=>r.type==="password");i&&(n=await h.unwrapWithFallback(i.data,t,i.salt,i.iv))}catch{console.log("Provided password invalid for unlock")}if(!n){const i=await j("Decrypt for Export","Provide the ORIGINAL password to decrypt for export:",{inputType:"password",placeholder:"Enter password..."});if(!i)return null;try{const r=e.keys.find(s=>s.type==="password");n=await h.unwrapWithFallback(r.data,i,r.salt,r.iv),o=i}catch{return await v("Error","Incorrect password"),null}}return{buffer:await h.decryptData(n,e.iv,e.content),password:o}}async function De(){const e=R.files[0],t=document.getElementById("new-password").value;if(!e){await v("Required","Please select a file to protect");return}if(!t){await v("Required","Protection password is required");return}const n=["123456","password","12345678","admin","admin123","benz","qwerty","123456789","welcome","letmein","monkey","dragon","master","football","access","iloveyou","testing","security","vault","pass123","root","user","111111","000000","trustno1"],o=t.toLowerCase().trim();if((t.length<8||n.includes(o))&&!await q("⚠️ Weak Password Warning",`This password is short or common, which makes it less secure against offline dictionary attacks. However, Coralgenz Vault still protects it with 2,000,000 PBKDF2 rounds.

Do you want to proceed with this password?`))return;const i=document.getElementById("confirm-add"),r=i.innerText;i.innerText="SECURING PAYLOAD (5s)...",i.disabled=!0;const s=document.getElementById("protection-process-container"),c=document.getElementById("file-upload-zone"),l=document.getElementById("file-preview"),d=document.getElementById("inline-password-section"),u=document.getElementById("protection-complete-container"),y=document.getElementById("security-terminal-body"),g=document.getElementById("process-timer-display"),f=document.getElementById("scanner-status-text"),w=document.querySelector(".scanner-center-shield");c?.classList.add("hidden"),l?.classList.add("hidden"),d?.classList.add("hidden"),u?.classList.add("hidden"),s?.classList.remove("hidden"),["step-analysis","step-prep","step-kdf","step-encrypt","step-meta","step-finalize","step-output","step-verify"].forEach(x=>{k(x,"")}),w&&w.classList.remove("success"),f&&(f.textContent="SECURING"),y&&(y.innerHTML="");const p=Date.now(),m=5e3,E=setInterval(()=>{const x=Math.min(Date.now()-p,m),C=Math.floor(x/1e3).toString().padStart(2,"0"),A=Math.floor(x%1e3/10).toString().padStart(2,"0");g&&(g.textContent=`00:${C}.${A}`),x>=m&&clearInterval(E)},30);try{H(14,"FILE STRUCTURE ANALYSIS // MEMORY ENCLAVE..."),k("step-analysis","active"),b("[00:00.15] INITIATING ENCRYPTION PIPELINE // ZERO-KNOWLEDGE V2.6"),b(`[00:00.35] File: "${e.name}" [${ae(e.size)}] | Type: ${e.type||"application/octet-stream"}`),b(`[00:00.55] Enclave memory block allocated: ${e.size} bytes. Isolation confirmed.`);const x=h.detectDeviceCapabilities();b(`[00:00.65] Hardware Profile: ${x.concurrency} Cores, ${x.memory}GB RAM [Worker Offload: Active]`),await new Promise(B=>setTimeout(B,700)),k("step-analysis","completed"),H(28,"INITIALIZING CSPRNG ENTROPY POOL..."),k("step-prep","active"),b("[00:00.85] Generating 256-bit cryptographic salt from hardware CSPRNG...");const C=h.generateSalt(),A=await h.generateKey();b("[00:01.10] Nonce generation: 96-bit AES-GCM Initialization Vector created."),b("[00:01.30] Ephemeral entropy validated: entropy score = 0.998."),await new Promise(B=>setTimeout(B,700)),k("step-prep","completed"),H(45,"DERIVING KEY (DUAL-STAGE KDF 2,000,000 ROUNDS)..."),k("step-kdf","active"),b("[00:01.50] Stage 1: HMAC-SHA512 Pre-whitening with Domain-Separated Pepper V6..."),b("[00:01.70] Worker Offload: Initializing background thread for smooth 60fps UI..."),b("[00:01.90] Stage 2: Computing 2,000,000 PBKDF2 iterations (Anti-JohnTheRipper / Anti-Hashcat)...");const V=await h.deriveKeyAsyncWorker(t,C,2e6,h.MILSPEC_ANTI_CRACKER_PEPPER_V6);b("[00:02.15] Key derivation complete: 256-bit symmetric cipher key established."),await new Promise(B=>setTimeout(B,900)),k("step-kdf","completed"),H(65,"AES-GCM-256 CIPHER STREAM PROCESSING..."),k("step-encrypt","active"),b("[00:02.45] Executing client-side WebCrypto AES-GCM 256-bit cipher...");const $=await e.arrayBuffer(),Q=await h.computePayloadHash($);b(`[00:02.65] SHA-256 Payload Integrity Seal: ${Q.substring(0,16)}... [VERIFIED]`);const Ee=$.slice(0),{iv:xe,ciphertext:ee}=await h.encryptData(A,$);b(`[00:02.85] Encrypting ${ae(e.size)} payload blocks into zero-knowledge ciphertext...`),b(`[00:03.15] Ciphertext generated (${ee.byteLength} bytes). 128-bit Galois Tag verified.`),await new Promise(B=>setTimeout(B,1e3)),k("step-encrypt","completed"),H(80,"PACKAGING ZERO-KNOWLEDGE METADATA..."),k("step-meta","active"),k("step-finalize","active"),b("[00:03.40] Wrapping master file key with AES key wrap cipher...");const{iv:Ve,wrappedData:$e}=await h.wrapKey(A,V),Ge=[{type:"password",salt:C,iv:Ve,data:$e}],ke={id:crypto.randomUUID(),name:e.name,type:e.type,size:e.size,date:Date.now(),authMode:"always",keys:Ge,content:ee,iv:xe,viewCount:0,expires:null,note:"",integrityHash:Q,accessLog:[{action:"created",date:Date.now()}]};await I.saveFile(ke),b("[00:03.75] Encrypted container committed to zero-knowledge local IndexedDB."),await new Promise(B=>setTimeout(B,700)),k("step-meta","completed"),k("step-finalize","completed"),H(92,"COMPILING STANDALONE HTML RUNTIME (.secure.html)..."),k("step-output","active"),b("[00:04.10] Assembling self-contained portable decryption engine..."),b("[00:04.35] Embedding browser-native WebCrypto decryptor payload..."),b("[00:04.55] Enforced security policy configured: ALWAYS ASK PASSWORD."),await new Promise(B=>setTimeout(B,650)),k("step-output","completed"),H(100,"CRYPTOGRAPHIC INTEGRITY VERIFIED [SEALED]"),k("step-verify","active"),b("[00:04.80] Authenticity check: GMAC integrity tag valid. Zero-knowledge verification OK."),b("[00:05.00] CRYPTOGRAPHIC CONTAINER LOCKED & SEALED. READY."),await new Promise(B=>setTimeout(B,350)),k("step-verify","completed"),clearInterval(E),g&&(g.textContent="00:05.00"),f&&(f.textContent="SECURED [✓]"),w&&w.classList.add("success"),await new Promise(B=>setTimeout(B,300)),s?.classList.add("hidden"),u?.classList.remove("hidden");const Ie=document.getElementById("complete-file-name");Ie&&(Ie.textContent=e.name),N={record:ke,buffer:Ee,password:t},S()}catch(x){clearInterval(E),console.error(x),b(`CRITICAL ERROR: ${x.message}`),await v("Error","Encryption failed: "+x.message),s?.classList.add("hidden"),l?.classList.remove("hidden"),d?.classList.remove("hidden")}finally{i.innerText=r,i.disabled=!1}}async function me(e){if(!Ue(e))try{const t=await I.getFile(e);if(!t){await v("Error","File not found");return}he=t,document.getElementById("auth-file-name").innerText=t.name,Y.showModal()}catch(t){console.error(t),await v("Error","Error opening file")}}async function Ne(){const e=he;if(!e||Ue(e.id))return;const t=document.getElementById("auth-password").value;if(!t)return;const n=document.getElementById("confirm-auth");n.innerText="Unlocking...";try{const o=e.keys.find(i=>i.type==="password");if(!o)throw new Error("Corrupt key data");const a=await h.unwrapWithFallback(o.data,t,o.salt,o.iv);Ft(e.id),Y.close(),document.getElementById("auth-password").value="",Zt(e,a)}catch(o){console.error(o),Kt(e.id),await v("Error","Incorrect password or decryption error.")}finally{n.innerText="Unlock"}}async function Zt(e,t){try{const n=await h.decryptData(t,e.iv,e.content),o=new Blob([n],{type:e.type});D=URL.createObjectURL(o);const a=document.getElementById("viewer-content");a.innerHTML="";const i=ve(e.name,e.type);_&&(i?(_.classList.remove("hidden"),_.onclick=()=>{if(!D)return;const p=document.createElement("a");p.style.display="none",p.href=D,p.download=e.name,document.body.appendChild(p),p.click(),document.body.removeChild(p)}):(_.classList.add("hidden"),_.onclick=null));const r=(e.name||"").toLowerCase(),s=(e.type||"").toLowerCase(),c=s.startsWith("image/")||/\.(jpg|jpeg|png|gif|webp|svg|bmp|ico)$/i.test(r),l=s.startsWith("video/")||/\.(mp4|webm|mov|mkv|ogg)$/i.test(r),d=s.startsWith("audio/")||/\.(mp3|wav|ogg|aac|m4a|flac)$/i.test(r),u=s==="application/pdf"||/\.pdf$/i.test(r),y=s.includes("excel")||s.includes("spreadsheet")||/\.(xlsx|xls|csv|tsv)$/i.test(r),g=s.includes("word")||/\.(docx|doc)$/i.test(r),f=s.includes("presentation")||s.includes("powerpoint")||/\.(pptx|ppt|pps|ppsx)$/i.test(r),w=s.startsWith("text/")||/\.(txt|json|js|ts|html|css|py|c|cpp|h|java|sh|xml|yaml|yml|sql|md|log|env|rs|go|kt|swift|rb|php)$/i.test(r);if(c){const p=document.createElement("img");p.src=D,p.style.maxWidth="90vw",p.style.maxHeight="80vh",p.style.objectFit="contain",a.appendChild(p)}else if(l||d){const p=document.createElement(l?"video":"audio");p.src=D,p.controls=!0,p.autoplay=!0,i||(p.setAttribute("controlsList","nodownload"),p.oncontextmenu=m=>m.preventDefault()),a.appendChild(p)}else if(u){const p=document.createElement("iframe");p.src=D+(i?"":"#toolbar=0"),p.style.width="100%",p.style.height="100%",p.style.border="none",a.appendChild(p)}else if(y)bt(n,a);else if(g)await Et(n,a);else if(f)await xt(n,e.name,a);else if(w){const m=new TextDecoder().decode(n),E=document.createElement("div");E.className="excel-viewer",E.style.background="#0f172a",E.style.color="#e2e8f0",E.innerHTML=`
      ...
      `,a.appendChild(E),E.querySelector("#vault-copy-code-btn")?.addEventListener("click",async()=>{await navigator.clipboard.writeText(m),await v("Success","Text copied to clipboard!")})}else{const p=document.createElement("div");p.className="ppt-slide-card",p.style.alignItems="center",p.style.justifyContent="center",p.style.textAlign="center",p.innerHTML=`
        <div style="font-size:48px;margin-bottom:12px;">📁</div>
