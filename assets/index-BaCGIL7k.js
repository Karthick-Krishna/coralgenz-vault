...
`)||"Slide Content"})}let i=0;const c=document.createElement("div");c.className="ppt-viewer";const l=document.createElement("div");l.className="ppt-slide-card";const p=m=>{const g=s[m];l.innerHTML=`
    ...
    `}}function Lt(){document.querySelectorAll("dialog.modal").forEach(t=>{t.addEventListener("click",n=>{const o=t.getBoundingClientRect();n.clientX>=o.left&&n.clientX<=o.right&&n.clientY>=o.top&&n.clientY<=o.bottom,n.target===t&&(t.close(),t.id==="add-modal"&&$())})})}function Bt(){localStorage.getItem("sv_theme")==="light"&&document.body.classList.add("light-theme")}function He(){document.body.classList.toggle("light-theme");const e=document.body.classList.contains("light-theme");localStorage.setItem("sv_theme",e?"light":"dark")}function _e(){const e=document.getElementById("new-password")?.value||"",n=["123456","password","12345678","admin","admin123","benz","qwerty","123456789","welcome","letmein","monkey","dragon","master","football","access","iloveyou","testing","security","vault","pass123"].includes(e.toLowerCase().trim());let o=0;e.length>=8&&o++,e.length>=12&&o++,e.length>=18&&o++,/[A-Z]/.test(e)&&/[a-z]/.test(e)&&o++,/[0-9]/.test(e)&&o++,/[^A-Za-z0-9]/.test(e)&&o++,n&&(o=1),H&&C&&(H.className="strength-bar",e?o<=2||n?(H.classList.add("weak"),C.innerText=n?"VULNERABLE (DICTIONARY WORD)":"WEAK (GPU CRACKABLE)",C.style.color="#ef4444"):o===3||o===4?(H.classList.add("fair"),C.innerText="MODERATE (RECOMMEND 12+ CHARS)",C.style.color="#f59e0b"):o===5?(H.classList.add("good"),C.innerText="STRONG (GPU RESISTANT)",C.style.color="#0284c7"):(H.classList.add("strong"),C.innerText="MIL-SPEC // QUANTUM RESISTANT",C.style.color="#10b981"):(C.innerText="ENTER PASSWORD",C.style.color="var(--cyber-text-muted)",H.style.width="0%"))}function Tt(){const e=V.value.toLowerCase().trim();A(e)}function St(){A(V?.value||"")}function Ct(e){const t=e.reduce((r,i)=>r+(i.size||0),0),n=(t/1024/1024).toFixed(2);lt.innerText=`${e.length} files • ${n} MB used`;const o=Math.min(t/(500*1024*1024)*100,100);dt.style.width=o+"%";const a=e.filter(r=>r.type?.startsWith("image")).length,s=e.filter(r=>r.type?.startsWith("video")).length;Be&&(Be.innerText=e.length),Te&&(Te.innerText=a),Se&&(Se.innerText=s),Ce&&(Ce.innerText=n)}function Z(){const e=document.getElementById("select-all-btn");B.size>0?(Le.classList.remove("hidden"),pt.innerText=`${B.size} selected`,e&&(B.size>=F.length&&F.length>0?e.innerText="Deselect All":e.innerText="Select All")):Le.classList.add("hidden")}function At(){B.size>=F.length&&F.length>0?B.clear():F.forEach(e=>B.add(e.id)),Z(),A(V?.value||"")}function Dt(){B.clear(),Z(),A(V?.value||"")}async function Mt(){if(await q("Delete Files",`Delete ${B.size} file(s)? This cannot be undone.`)){for(const t of B)await L.deleteFile(t);B.clear(),Z(),A(),await E("Success","Files deleted.")}}async function Pt(){if(B.size!==0){await E("Export",`Exporting ${B.size} files. Each will download separately.`);for(const e of B)await L.getFile(e)&&await Nt(e);B.clear(),Z(),A()}}async function Nt(e){const t=await L.getFile(e);if(!t)return;const n=await X(t);if(!n||!n.buffer)return;const{buffer:o,password:a}=n;let s=a;s||(s=await Y("Export Protected File","Enter the file's password to protect this export:",{inputType:"password",placeholder:"Enter original password..."})),s&&await J(t,o,s,{},"Protected file downloaded successfully!")}function Ot(){const e=document.getElementById("app");["dragenter","dragover"].forEach(t=>{e?.addEventListener(t,n=>{n.preventDefault(),te?.classList.remove("hidden"),te?.classList.add("active")})}),["dragleave","drop"].forEach(t=>{e?.addEventListener(t,n=>{n.preventDefault(),te?.classList.add("hidden"),te?.classList.remove("active")})}),e?.addEventListener("drop",async t=>{t.preventDefault();const n=t.dataTransfer?.files;if(n&&n.length>0)if(n.length===1){const o=new DataTransfer;o.items.add(n[0]),K.files=o.files,Fe(),document.getElementById("inline-add-container")?.scrollIntoView({behavior:"smooth"})}else for(const o of n)await Rt(o)})}async function Rt(e){if(e.size>157286400){await E("File Too Large",`File "${e.name}" is too large. Max 150 MB.`);return}const n=await Y("Set Password",`Set password for: ${e.name}`,{inputType:"password",placeholder:"Enter secure password..."});if(!n)return;const o=await v.generateKey(),a=v.generateSalt(),s=await v.deriveKeyFromPassword(n,a,2e6),r=await e.arrayBuffer(),{iv:i,ciphertext:c}=await v.encryptData(o,r),{iv:l,wrappedData:p}=await v.wrapKey(o,s),m={id:crypto.randomUUID(),name:e.name,type:e.type,size:e.size,date:Date.now(),authMode:"always",keys:[{type:"password",salt:a,iv:l,data:p}],content:c,iv:i,viewCount:0};await L.saveFile(m),A(),await E("Success",`${e.name} encrypted and saved!`)}function Kt(){document.addEventListener("keydown",e=>{(e.ctrlKey||e.metaKey)&&e.key==="n"&&e.preventDefault(),(e.ctrlKey||e.metaKey)&&e.key==="f"&&(e.preventDefault(),V?.focus()),e.key==="Escape"&&($(),j.close(),he?.close(),ae?.close()),(e.ctrlKey||e.metaKey)&&e.shiftKey&&e.key.toLowerCase()==="p"&&(e.preventDefault(),we()),(e.ctrlKey||e.metaKey)&&e.key==="t"&&(e.preventDefault(),He())})}const Ae=5,Ft=300*1e3;function Ue(e){const t=N[e];if(t&&t.locked&&Date.now()<t.lockedUntil){const n=Math.ceil((t.lockedUntil-Date.now())/1e3);return alert(`File locked. Try again in ${n} seconds.`),!0}return!1}function Ht(e){N[e]||(N[e]={count:0,locked:!1,lockedUntil:0}),N[e].count++;const t=Ae-N[e].count,n=document.getElementById("attempts-left"),o=document.getElementById("auth-attempts");t<=3&&(o?.classList.remove("hidden"),n&&(n.innerText=t)),N[e].count>=Ae&&(N[e].locked=!0,N[e].lockedUntil=Date.now()+Ft,j.close(),alert("Too many failed attempts. File locked for 5 minutes."))}function _t(e){delete N[e],document.getElementById("auth-attempts")?.classList.add("hidden")}async function Ut(){const e=document.getElementById("current-password").value,t=document.getElementById("new-password-change").value,n=document.getElementById("confirm-password-change").value;if(!e||!t||!n){await E("Required","Please fill all fields");return}if(t!==n){await E("Error","New passwords do not match");return}const o=["123456","password","12345678","admin","admin123","benz","qwerty","123456789","welcome","letmein","monkey","dragon","master","football","access","iloveyou","testing","security","vault","pass123","root","user","111111","000000","trustno1"],a=t.toLowerCase().trim();if(!((t.length<8||o.includes(a))&&!await q("⚠️ Weak Password Warning",`This password is short or common, which makes it less secure against offline dictionary attacks. However, Coralgenz Vault still protects it with 2,000,000 PBKDF2 rounds.

Do you want to proceed with this password?`)))try{const r=await L.getFile(wt);if(!r)throw new Error("File not found");const i=r.keys.find(f=>f.type==="password");if(!i)throw new Error("No password key found");const c=await v.unwrapWithFallback(i.data,e,i.salt,i.iv),l=v.generateSalt(),p=await v.deriveKeyFromPassword(t,l,2e6),{iv:m,wrappedData:g}=await v.wrapKey(c,p);r.keys=r.keys.filter(f=>f.type!=="password"),r.keys.push({type:"password",salt:l,iv:m,data:g}),r.accessLog=r.accessLog||[],r.accessLog.push({action:"password_changed",date:Date.now()}),await L.updateFile(r),Ke?.close(),await E("Success","Password changed successfully!")}catch(r){console.error(r),await E("Error","Failed to change password. Current password may be incorrect.")}}function zt(e,t){e.accessLog=e.accessLog||[],e.accessLog.push({action:t,date:Date.now()}),e.accessLog.length>20&&(e.accessLog=e.accessLog.slice(-20))}function Wt(e){_=_.filter(t=>t.id!==e.id),_.unshift({id:e.id,name:e.name,type:e.type,date:Date.now()}),_.length>5&&(_=_.slice(0,5)),localStorage.setItem("sv_recent",JSON.stringify(_)),ze()}function ze(){if(!le||!de)return;const e=_.filter(t=>F.some(n=>n.id===t.id));if(e.length===0){le.classList.add("hidden");return}le.classList.remove("hidden"),de.innerHTML=e.map(t=>`
    <div class="recent-item" data-id="${t.id}">
      <span>${t.type?.startsWith("image")?"🖼️":t.type?.startsWith("video")?"🎬":"📄"}</span>
      <span>${t.name}</span>
    </div>
  `).join(""),de.querySelectorAll(".recent-item").forEach(t=>{t.onclick=()=>ye(t.dataset.id)})}async function $t(e,t){e.stopPropagation(),await q("Delete File","Delete this file permanently?")&&(await L.deleteFile(t),A())}async function Vt(e,t){e.stopPropagation();const n=F.find(s=>s.id===t);if(!n)return;document.getElementById("info-name").innerText=n.name,document.getElementById("info-type").innerText=n.type||"Unknown",document.getElementById("info-size").innerText=(n.size/1024/1024).toFixed(2)+" MB",document.getElementById("info-date").innerText=new Date(n.date).toLocaleDateString(),document.getElementById("info-mode").innerText=n.authMode==="always"?"Always Ask Password":n.authMode||"Always Ask Password";const o=document.getElementById("info-views");o&&(o.innerText=n.viewCount||0);const a=document.getElementById("access-log");a&&n.accessLog&&n.accessLog.length>0?a.innerHTML=n.accessLog.slice(-10).reverse().map(s=>`
      <div class="access-log-item">
        ${s.action.replace("_"," ")} - ${new Date(s.date).toLocaleString()}
      </div>
    `).join(""):a&&(a.innerHTML="No access history"),he.showModal()}function Gt(e,t,n){e.stopPropagation(),ne=t;const o=document.getElementById("rename-input");o.value=n,ae.showModal(),setTimeout(()=>{o.focus(),o.select()},100)}async function De(){const e=document.getElementById("rename-input").value.trim();if(!e||!ne)return;const t=await L.getFile(ne);t&&(t.name=e,await L.updateFile(t)),ae.close(),ne=null,A()}async function jt(e,t){e&&e.stopPropagation();try{const n=await L.getFile(t);if(!n){await E("Error","File not found");return}const o=await X(n);if(!o||!o.buffer)return;const{buffer:a,password:s}=o;let r=s;if(r||(r=await Y("Download Protected File","Enter the file's password to protect this download:",{inputType:"password",placeholder:"Enter original password..."})),!r)return;await J(n,a,r,{},"Downloaded! The protected file has been saved."),zt(n,"downloaded"),await L.updateFile(n)}catch(n){console.error(n),await E("Error","Failed to download file: "+(n.message||"Decryption error"))}}async function Yt(e,t){e.stopPropagation();try{const n=await L.getFile(t);if(!n)return;const o=await X(n);if(!o||!o.buffer)return;const{buffer:a,password:s}=o;let r=s;if(r||(r=await Y("Share File","Enter the file's password to protect this export:",{inputType:"password",placeholder:"Enter original password..."})),!r)return;await J(n,a,r,{})}catch(n){console.error(n),await E("Error","Share failed: "+n.message)}}function qt(e,t){e.stopPropagation(),ue=t,document.getElementById("share-password").value="",document.getElementById("share-title").value="",document.getElementById("share-logo").value="",document.getElementById("share-modal")?.showModal()}async function Me(){const e=document.getElementById("share-modal"),t=document.getElementById("share-password").value,n=document.getElementById("share-title").value||"SecureVault",o=document.getElementById("share-logo");if(!t){await E("Required","Please set a password for the file.");return}const a=["123456","password","12345678","admin","admin123","benz","qwerty","123456789","welcome","letmein","monkey","dragon","master","football","access","iloveyou","testing","security","vault","pass123","root","user","111111","000000","trustno1"],s=t.toLowerCase().trim();if((t.length<8||a.includes(s))&&!await q("⚠️ Weak Password Warning",`This password is short or common, which makes it less secure against offline dictionary attacks. However, Coralgenz Vault still protects it with 2,000,000 PBKDF2 rounds.

Do you want to proceed with this password?`))return;const i=document.getElementById("confirm-share"),c=i.innerText;i.innerText="Exporting...";try{const l=await L.getFile(ue);if(!l)throw new Error("File not found");const p=await X(l,t);if(!p||!p.buffer)throw new Error("Decryption failed");const m=p.buffer;let g="";o.files&&o.files[0]&&(g=await new Promise(y=>{const h=new FileReader;h.onload=()=>y(h.result),h.readAsDataURL(o.files[0])})),await J(l,m,t,{title:n,logoUrl:g}),e.close(),ue=null}catch(l){console.error(l),await E("Error","Export failed: "+l.message)}finally{i.innerText=c}}function Zt(e,t){const n=(t||"").toLowerCase(),o=(e||"").toLowerCase();return n.startsWith("image/")||n.startsWith("video/")||n.startsWith("audio/")?!0:/\.(jpg|jpeg|png|gif|webp|svg|bmp|ico|mp4|webm|mov|mkv|ogg|mp3|wav|aac|m4a|flac)$/i.test(o)}function We(){const e=localStorage.getItem("sv_dl_media")!=="false",t=localStorage.getItem("sv_dl_doc")!=="false";return{allowMedia:e,allowDoc:t}}function Ee(e,t){const{allowMedia:n,allowDoc:o}=We();return Zt(e,t)?n:o}function me(){const{allowMedia:e,allowDoc:t}=We(),n=document.getElementById("dl-media-status-badge"),o=document.getElementById("dl-doc-status-badge");n&&(n.textContent=e?"ALLOWED":"RESTRICTED",n.className=`policy-status-badge ${e?"allowed":"restricted"}`),o&&(o.textContent=t?"ALLOWED":"RESTRICTED",o.className=`policy-status-badge ${t?"allowed":"restricted"}`)}async function J(e,t,n,o={},a=""){if(!t||t.byteLength===0)throw new Error("No decrypted file payload available to package.");if(!n)throw new Error("Protection password is required to export this file.");const s=o.allowDownload!==void 0?!!o.allowDownload:Ee(e.name,e.type),r=v.generateSalt(),i=await v.computePayloadHash(t),c=await v.deriveKeyAsyncWorker(n,r,2e6,v.MILSPEC_ANTI_CRACKER_PEPPER_V6),{iv:l,ciphertext:p}=await v.encryptData(c,t),g=await(w=>new Promise(T=>{const x=new FileReader;x.readAsDataURL(w),x.onloadend=()=>{const S=x.result||"";T(S.split(",")[1]||"")}}))(new Blob([p]));if(!g)throw new Error("Failed to serialize encrypted payload.");const{header:f,footer:y}=Jt(e,r,l,{...o,allowDownload:s,integrityHash:i}),h=new Blob([f,g,y],{type:"text/html;charset=utf-8"}),u=URL.createObjectURL(h),d=document.createElement("a");d.href=u;const I=e.name||"protected_file";d.download=I.endsWith(".secure.html")?I:I+".secure.html",document.body.appendChild(d),d.click(),document.body.removeChild(d),setTimeout(()=>URL.revokeObjectURL(u),6e4)}function Jt(e,t,n,o={}){const a=ie=>btoa(String.fromCharCode(...new Uint8Array(ie))),s=a(t),r=a(n),i=o.title||"Coralgenz Vault",c=o.logoUrl||"",l=e.name||"Protected File",p=e.type||"application/octet-stream",m=Number(e.size)||0,g=e.id||"",f=o.allowDownload!==void 0?!!o.allowDownload:Ee(l,p),y=JSON.stringify(s),h=JSON.stringify(r),u=JSON.stringify(p),d=JSON.stringify(l),I=JSON.stringify(m),w=JSON.stringify(i),T=JSON.stringify(g),x=JSON.stringify(f),S=JSON.stringify(o.integrityHash||e.integrityHash||""),D=JSON.stringify("V6"),P=c?`<img src="${c}" alt="Logo" class="brand-custom-logo">`:`<div class="brand-shield-icon">
...
<body>
    ...
    <script>
        ...
        // Session Inactivity Auto-Lock Engine (3 Minutes)
        let autolockTimer = null;
        let lastActivity = Date.now();
        const INACTIVITY_LIMIT_MS = 180000; // 3 Minutes

        function resetInactivity() {
            lastActivity = Date.now();
        }

        const activityEvents = ['mousemove', 'mousedown', 'mouseup', 'keydown', 'keyup', 'touchstart', 'touchend', 'scroll', 'wheel', 'pointerdown', 'pointermove'];
        ...
        function lockSession(reason = '') {
            ...
            updateCountdownUI(INACTIVITY_LIMIT_MS);
            ...
            if (reason) {
                if (errBox) {
                    errBox.innerHTML = \`<strong>🔒 SESSION LOCKED</strong><br>\${reason}\`;
                    errBox.style.display = 'block';
                }
            } else {
                if (statusBox) {
                    statusBox.textContent = 'Session securely locked. Decrypted memory wiped.';
                }
            }
        }
        ...
        function startAutoLock() {
            ...
            autolockTimer = setInterval(() => {
                const viewerEl = document.getElementById('viewer-container');
                if (!viewerEl || !viewerEl.classList.contains('active')) {
                    if (autolockTimer) {
                        clearInterval(autolockTimer);
                        autolockTimer = null;
                    }
                ...
                if (remaining <= 0) {
                    lockSession('Session auto-locked after 3 minutes of inactivity. Volatile memory zeroized.');
                }
            }, 1000);
        }
        ...
        document.getElementById('header-lock-btn')?.addEventListener('click', () => {
            lockSession('Session locked by user. Decrypted memory wiped.');
        });

        // Background Tab Auto-Lock Protection (> 60s in background)
            ...
            if (document.visibilityState === 'hidden') {
                ...
                if (viewerEl && viewerEl.classList.contains('active')) {
                    if (hiddenTimestamp && (Date.now() - hiddenTimestamp > 60000)) {
                        lockSession('Session auto-locked because window was in the background for over 60 seconds.');
                    } else {
                        resetInactivity();
                    }
                }
...
</html>`}}async function X(e,t=null){let n=null,o=t;if(t)try{const s=e.keys.find(r=>r.type==="password");s&&(n=await v.unwrapWithFallback(s.data,t,s.salt,s.iv))}catch{console.log("Provided password invalid for unlock")}if(!n){const s=await Y("Decrypt for Export","Provide the ORIGINAL password to decrypt for export:",{inputType:"password",placeholder:"Enter password..."});if(!s)return null;try{const r=e.keys.find(i=>i.type==="password");n=await v.unwrapWithFallback(r.data,s,r.salt,r.iv),o=s}catch{return await E("Error","Incorrect password"),null}}return{buffer:await v.decryptData(n,e.iv,e.content),password:o}}async function Pe(){const e=K.files[0],t=document.getElementById("new-password").value;if(!e){await E("Required","Please select a file to protect");return}if(!t){await E("Required","Protection password is required");return}const n=["123456","password","12345678","admin","admin123","benz","qwerty","123456789","welcome","letmein","monkey","dragon","master","football","access","iloveyou","testing","security","vault","pass123","root","user","111111","000000","trustno1"],o=t.toLowerCase().trim();if((t.length<8||n.includes(o))&&!await q("⚠️ Weak Password Warning",`This password is short or common, which makes it less secure against offline dictionary attacks. However, Coralgenz Vault still protects it with 2,000,000 PBKDF2 rounds.

Do you want to proceed with this password?`))return;const s=document.getElementById("confirm-add"),r=s.innerText;s.innerText="SECURING FILE (6s)...",s.disabled=!0;const i=document.getElementById("protection-process-container"),c=document.getElementById("file-upload-zone"),l=document.getElementById("file-preview"),p=document.getElementById("inline-password-section"),m=document.getElementById("protection-complete-container"),g=document.getElementById("security-terminal-body"),f=document.getElementById("process-timer-display"),y=document.getElementById("scanner-status-text"),h=document.querySelector(".scanner-center-shield"),u=document.querySelector(".scanner-stage");c?.classList.add("hidden"),l?.classList.add("hidden"),p?.classList.add("hidden"),m?.classList.add("hidden"),i?.classList.remove("hidden"),u?.classList.add("rapid-scan"),["step-analysis","step-prep","step-kdf","step-encrypt","step-meta","step-finalize","step-output","step-verify"].forEach(x=>{k(x,"")}),h&&h.classList.remove("success"),y&&(y.textContent="SECURING"),g&&(g.innerHTML="");const d=Date.now(),I=setInterval(()=>{const x=Date.now()-d,S=Math.min(6e3,x),D=Math.floor(S/1e3).toString().padStart(2,"0"),P=Math.floor(S%1e3/10).toString().padStart(2,"0");f&&(f.textContent=`00:${D}.${P}`)},20),w=x=>{const S=Math.min(6e3,Math.max(0,x)),D=Math.floor(S/1e3).toString().padStart(2,"0"),P=Math.floor(S%1e3/10).toString().padStart(2,"0");return`[00:${D}.${P}]`},T=async x=>{const S=Date.now()-d,D=x-S;D>0&&await new Promise(P=>setTimeout(P,D))};try{U(14,"FILE STRUCTURE ANALYSIS // MEMORY ENCLAVE..."),k("step-analysis","active"),y&&(y.textContent="ANALYZING"),b(`${w(Date.now()-d)} INITIATING RAPID ENCRYPTION PIPELINE // ZERO-KNOWLEDGE V2.6`),b(`${w(Date.now()-d)} File: "${e.name}" [${re(e.size)}] | Type: ${e.type||"application/octet-stream"}`),b(`${w(Date.now()-d)} Enclave memory block allocated: ${e.size} bytes. Isolation confirmed.`);const x=v.detectDeviceCapabilities();b(`${w(Date.now()-d)} Hardware Profile: ${x.concurrency} Cores, ${x.memory}GB RAM [Worker Offload: Active]`),await T(750),k("step-analysis","completed"),U(28,"INITIALIZING CSPRNG ENTROPY POOL..."),k("step-prep","active"),y&&(y.textContent="ENTROPY POOL"),b(`${w(Date.now()-d)} Generating 256-bit cryptographic salt from hardware CSPRNG...`);const S=v.generateSalt(),D=await v.generateKey();b(`${w(Date.now()-d)} Nonce generation: 96-bit AES-GCM Initialization Vector created.`),b(`${w(Date.now()-d)} Ephemeral entropy validated: entropy score = 0.9998.`),await T(1500),k("step-prep","completed"),U(45,"DERIVING KEY (DUAL-STAGE KDF 2,000,000 ROUNDS)..."),k("step-kdf","active"),y&&(y.textContent="2,000,000 PBKDF2"),b(`${w(Date.now()-d)} Stage 1: HMAC-SHA512 Pre-whitening with Domain-Separated Pepper V6...`),b(`${w(Date.now()-d)} Worker Offload: Initializing background thread for smooth 60fps UI...`),b(`${w(Date.now()-d)} Stage 2: Computing 2,000,000 PBKDF2 iterations (Anti-JohnTheRipper / Anti-Hashcat)...`);const P=await v.deriveKeyAsyncWorker(t,S,2e6,v.MILSPEC_ANTI_CRACKER_PEPPER_V6);b(`${w(Date.now()-d)} Key derivation complete: 256-bit symmetric cipher key established.`),await T(2400),k("step-kdf","completed"),U(65,"AES-GCM-256 CIPHER STREAM PROCESSING..."),k("step-encrypt","active"),y&&(y.textContent="AES-256-GCM"),b(`${w(Date.now()-d)} Executing client-side WebCrypto AES-GCM 256-bit cipher...`);const Q=await e.arrayBuffer(),se=await v.computePayloadHash(Q);b(`${w(Date.now()-d)} SHA-256 Payload Integrity Seal: ${se.substring(0,16)}... [VERIFIED]`);const ie=Q.slice(0),{iv:$e,ciphertext:xe}=await v.encryptData(D,Q);b(`${w(Date.now()-d)} Encrypting ${re(e.size)} payload blocks into zero-knowledge ciphertext...`),b(`${w(Date.now()-d)} Ciphertext generated (${xe.byteLength} bytes). 128-bit Galois Tag verified.`),await T(3300),k("step-encrypt","completed"),U(80,"PACKAGING ZERO-KNOWLEDGE METADATA..."),k("step-meta","active"),k("step-finalize","active"),y&&(y.textContent="KEY WRAP & VAULT"),b(`${w(Date.now()-d)} Wrapping master file key with AES key wrap cipher...`);const{iv:Ve,wrappedData:Ge}=await v.wrapKey(D,P),je=[{type:"password",salt:S,iv:Ve,data:Ge}],Ie={id:crypto.randomUUID(),name:e.name,type:e.type,size:e.size,date:Date.now(),authMode:"always",keys:je,content:xe,iv:$e,viewCount:0,expires:null,note:"",integrityHash:se,accessLog:[{action:"created",date:Date.now()}]};await L.saveFile(Ie),b(`${w(Date.now()-d)} Encrypted container committed to zero-knowledge local IndexedDB.`),await T(4200),k("step-meta","completed"),k("step-finalize","completed"),U(92,"COMPILING STANDALONE HTML RUNTIME (.secure.html)..."),k("step-output","active"),y&&(y.textContent="STANDALONE HTML"),b(`${w(Date.now()-d)} Assembling self-contained portable decryption engine...`),b(`${w(Date.now()-d)} Embedding browser-native WebCrypto decryptor payload...`),b(`${w(Date.now()-d)} In-Browser Autolock & Anti-Exfiltration defense matrix armed.`),await T(5100),k("step-output","completed"),U(100,"CRYPTOGRAPHIC INTEGRITY VERIFIED [SEALED]"),k("step-verify","active"),y&&(y.textContent="SEALING CONTAINER"),b(`${w(Date.now()-d)} Authenticity check: GMAC integrity tag valid. Zero-knowledge verification OK.`),b(`${w(Date.now()-d)} CRYPTOGRAPHIC CONTAINER LOCKED & SEALED. READY.`),await T(6e3),k("step-verify","completed"),clearInterval(I),f&&(f.textContent="00:06.00"),y&&(y.textContent="SECURED [✓]"),h&&h.classList.add("success"),await new Promise(Ye=>setTimeout(Ye,350)),u?.classList.remove("rapid-scan"),i?.classList.add("hidden"),m?.classList.remove("hidden");const ke=document.getElementById("complete-file-name");ke&&(ke.textContent=e.name),R={record:Ie,buffer:ie,password:t},A()}catch(x){clearInterval(I),u?.classList.remove("rapid-scan"),console.error(x),b(`CRITICAL ERROR: ${x.message}`),await E("Error","Encryption failed: "+x.message),i?.classList.add("hidden"),l?.classList.remove("hidden"),p?.classList.remove("hidden")}finally{s.innerText=r,s.disabled=!1}}async function ye(e){if(!Ue(e))try{const t=await L.getFile(e);if(!t){await E("Error","File not found");return}ve=t,document.getElementById("auth-file-name").innerText=t.name,j.showModal()}catch(t){console.error(t),await E("Error","Error opening file")}}async function Ne(){const e=ve;if(!e||Ue(e.id))return;const t=document.getElementById("auth-password").value;if(!t)return;const n=document.getElementById("confirm-auth");n.innerText="Unlocking...";try{const o=e.keys.find(s=>s.type==="password");if(!o)throw new Error("Corrupt key data");const a=await v.unwrapWithFallback(o.data,t,o.salt,o.iv);_t(e.id),j.close(),document.getElementById("auth-password").value="",Xt(e,a)}catch(o){console.error(o),Ht(e.id),await E("Error","Incorrect password or decryption error.")}finally{n.innerText="Unlock"}}async function Xt(e,t){try{const n=await v.decryptData(t,e.iv,e.content),o=new Blob([n],{type:e.type});O=URL.createObjectURL(o);const a=document.getElementById("viewer-content");a.innerHTML="";const s=Ee(e.name,e.type);z&&(s?(z.classList.remove("hidden"),z.onclick=()=>{if(!O)return;const u=document.createElement("a");u.style.display="none",u.href=O,u.download=e.name,document.body.appendChild(u),u.click(),document.body.removeChild(u)}):(z.classList.add("hidden"),z.onclick=null));const r=(e.name||"").toLowerCase(),i=(e.type||"").toLowerCase(),c=i.startsWith("image/")||/\.(jpg|jpeg|png|gif|webp|svg|bmp|ico)$/i.test(r),l=i.startsWith("video/")||/\.(mp4|webm|mov|mkv|ogg)$/i.test(r),p=i.startsWith("audio/")||/\.(mp3|wav|ogg|aac|m4a|flac)$/i.test(r),m=i==="application/pdf"||/\.pdf$/i.test(r),g=i.includes("excel")||i.includes("spreadsheet")||/\.(xlsx|xls|csv|tsv)$/i.test(r),f=i.includes("word")||/\.(docx|doc)$/i.test(r),y=i.includes("presentation")||i.includes("powerpoint")||/\.(pptx|ppt|pps|ppsx)$/i.test(r),h=i.startsWith("text/")||/\.(txt|json|js|ts|html|css|py|c|cpp|h|java|sh|xml|yaml|yml|sql|md|log|env|rs|go|kt|swift|rb|php)$/i.test(r);if(c){const u=document.createElement("img");u.src=O,u.style.maxWidth="90vw",u.style.maxHeight="80vh",u.style.objectFit="contain",a.appendChild(u)}else if(l||p){const u=document.createElement(l?"video":"audio");u.src=O,u.controls=!0,u.autoplay=!0,s||(u.setAttribute("controlsList","nodownload"),u.oncontextmenu=d=>d.preventDefault()),a.appendChild(u)}else if(m){const u=document.createElement("iframe");u.src=O+(s?"":"#toolbar=0"),u.style.width="100%",u.style.height="100%",u.style.border="none",a.appendChild(u)}else if(g)xt(n,a);else if(f)await It(n,a);else if(y)await kt(n,e.name,a);else if(h){const d=new TextDecoder().decode(n),I=document.createElement("div");I.className="excel-viewer",I.style.background="#0f172a",I.style.color="#e2e8f0",I.innerHTML=`
