...
<body>
    ...
    <\/script>
...
</html>`}}async function G(e,t=null){let n=null,o=t;if(t)try{const s=e.keys.find(r=>r.type==="password");if(s){const r=await g.deriveKeyFromPassword(t,s.salt);n=await g.unwrapKey(s.data,r,s.iv)}}catch{console.log("Provided password invalid for unlock")}if(!n){const s=await H("Decrypt for Export","Provide the ORIGINAL password to decrypt for export:",{inputType:"password",placeholder:"Enter password..."});if(!s)return null;try{const r=e.keys.find(c=>c.type==="password"),i=await g.deriveKeyFromPassword(s,r.salt);n=await g.unwrapKey(r.data,i,r.iv),o=s}catch{return await f("Error","Incorrect password"),null}}return{buffer:await g.decryptData(n,e.iv,e.content),password:o}}async function Le(){const e=D.files[0],t=document.getElementById("new-password").value;if(!e){await f("Required","Please select a file to protect");return}if(!t){await f("Required","Protection password is required");return}const n=document.getElementById("confirm-add"),o=n.innerText;n.innerText="SECURING PAYLOAD (5s)...",n.disabled=!0;const a=document.getElementById("protection-process-container"),s=document.getElementById("file-upload-zone"),r=document.getElementById("file-preview"),i=document.getElementById("inline-password-section"),c=document.getElementById("protection-complete-container"),l=document.getElementById("security-terminal-body"),d=document.getElementById("process-timer-display"),p=document.getElementById("scanner-status-text"),y=document.querySelector(".scanner-center-shield");s?.classList.add("hidden"),r?.classList.add("hidden"),i?.classList.add("hidden"),c?.classList.add("hidden"),a?.classList.remove("hidden"),["step-analysis","step-prep","step-kdf","step-encrypt","step-meta","step-finalize","step-output","step-verify"].forEach(u=>{v(u,"")}),y&&y.classList.remove("success"),p&&(p.textContent="SECURING"),l&&(l.innerHTML="");const L=Date.now(),I=5e3,h=setInterval(()=>{const u=Math.min(Date.now()-L,I),m=Math.floor(u/1e3).toString().padStart(2,"0"),k=Math.floor(u%1e3/10).toString().padStart(2,"0");d&&(d.textContent=`00:${m}.${k}`),u>=I&&clearInterval(h)},30);try{N(14,"FILE STRUCTURE ANALYSIS // MEMORY ENCLAVE..."),v("step-analysis","active"),w("[00:00.15] INITIATING ENCRYPTION PIPELINE // ZERO-KNOWLEDGE V2.6"),w(`[00:00.35] File: "${e.name}" [${ie(e.size)}] | Type: ${e.type||"application/octet-stream"}`),w(`[00:00.55] Enclave memory block allocated: ${e.size} bytes. Isolation confirmed.`),await new Promise(x=>setTimeout(x,700)),v("step-analysis","completed"),N(28,"INITIALIZING CSPRNG ENTROPY POOL..."),v("step-prep","active"),w("[00:00.85] Generating 128-bit cryptographic salt from hardware CSPRNG...");const u=g.generateSalt(),m=await g.generateKey();w("[00:01.10] Nonce generation: 96-bit AES-GCM Initialization Vector created."),w("[00:01.30] Ephemeral entropy validated: entropy score = 0.998."),await new Promise(x=>setTimeout(x,700)),v("step-prep","completed"),N(45,"DERIVING KEY (PBKDF2-SHA256 600,000 ROUNDS)..."),v("step-kdf","active"),w("[00:01.50] Deriving master cipher key via PBKDF2-HMAC-SHA256..."),w("[00:01.85] Computing 600,000 computational work-factor rounds...");const k=await g.deriveKeyFromPassword(t,u);w("[00:02.15] Key derivation complete: 256-bit symmetric cipher key established."),await new Promise(x=>setTimeout(x,900)),v("step-kdf","completed"),N(65,"AES-GCM-256 CIPHER STREAM PROCESSING..."),v("step-encrypt","active"),w("[00:02.45] Executing client-side WebCrypto AES-GCM 256-bit cipher...");const T=await e.arrayBuffer(),O=T.slice(0),{iv:$,ciphertext:ee}=await g.encryptData(m,T);w(`[00:02.85] Encrypting ${ie(e.size)} payload blocks into zero-knowledge ciphertext...`),w(`[00:03.15] Ciphertext generated (${ee.byteLength} bytes). 128-bit Galois Tag verified.`),await new Promise(x=>setTimeout(x,1e3)),v("step-encrypt","completed"),N(80,"PACKAGING ZERO-KNOWLEDGE METADATA..."),v("step-meta","active"),v("step-finalize","active"),w("[00:03.40] Wrapping master file key with AES key wrap cipher...");const{iv:ye,wrappedData:te}=await g.wrapKey(m,k),Fe=[{type:"password",salt:u,iv:ye,data:te}],he={id:crypto.randomUUID(),name:e.name,type:e.type,size:e.size,date:Date.now(),authMode:"always",keys:Fe,content:ee,iv:$,viewCount:0,expires:null,note:"",accessLog:[{action:"created",date:Date.now()}]};await E.saveFile(he),w("[00:03.75] Encrypted container committed to zero-knowledge local IndexedDB."),await new Promise(x=>setTimeout(x,700)),v("step-meta","completed"),v("step-finalize","completed"),N(92,"COMPILING STANDALONE HTML RUNTIME (.secure.html)..."),v("step-output","active"),w("[00:04.10] Assembling self-contained portable decryption engine..."),w("[00:04.35] Embedding browser-native WebCrypto decryptor payload..."),w("[00:04.55] Enforced security policy configured: ALWAYS ASK PASSWORD."),await new Promise(x=>setTimeout(x,650)),v("step-output","completed"),N(100,"CRYPTOGRAPHIC INTEGRITY VERIFIED [SEALED]"),v("step-verify","active"),w("[00:04.80] Authenticity check: GMAC integrity tag valid. Zero-knowledge verification OK."),w("[00:05.00] CRYPTOGRAPHIC CONTAINER LOCKED & SEALED. READY."),await new Promise(x=>setTimeout(x,350)),v("step-verify","completed"),clearInterval(h),d&&(d.textContent="00:05.00"),p&&(p.textContent="SECURED [✓]"),y&&y.classList.add("success"),await new Promise(x=>setTimeout(x,300)),a?.classList.add("hidden"),c?.classList.remove("hidden");const we=document.getElementById("complete-file-name");we&&(we.textContent=e.name),A={record:he,buffer:O,password:t},B()}catch(u){clearInterval(h),console.error(u),w(`CRITICAL ERROR: ${u.message}`),await f("Error","Encryption failed: "+u.message),a?.classList.add("hidden"),r?.classList.remove("hidden"),i?.classList.remove("hidden")}finally{n.innerText=o,n.disabled=!1}}async function le(e){try{const t=await E.getFile(e);if(!t){await f("Error","File not found");return}J=t,document.getElementById("auth-file-name").innerText=t.name,X.showModal()}catch(t){console.error(t),await f("Error","Error opening file")}}async function Ce(){const e=document.getElementById("auth-password").value;if(!J||!e)return;const t=document.getElementById("confirm-auth");t.innerText="Unlocking...";try{const n=J,o=n.keys.find(r=>r.type==="password");if(!o)throw new Error("Corrupt key data");const a=await g.deriveKeyFromPassword(e,o.salt),s=await g.unwrapKey(o.data,a,o.iv);X.close(),document.getElementById("auth-password").value="",At(n,s)}catch(n){console.error(n),await f("Error","Incorrect password or error.")}finally{t.innerText="Unlock"}}async function At(e,t){try{const n=await g.decryptData(t,e.iv,e.content),o=new Blob([n],{type:e.type});S=URL.createObjectURL(o);const a=document.getElementById("viewer-content");a.innerHTML="";const s=ge(e.name,e.type);if(R&&(s?(R.classList.remove("hidden"),R.onclick=()=>{if(!S)return;const r=document.createElement("a");r.style.display="none",r.href=S,r.download=e.name,document.body.appendChild(r),r.click(),document.body.removeChild(r)}):(R.classList.add("hidden"),R.onclick=null)),e.type.startsWith("image/")){const r=document.createElement("img");r.src=S,a.appendChild(r)}else if(e.type.startsWith("video/")||e.type.startsWith("audio/")){const r=document.createElement(e.type.startsWith("video/")?"video":"audio");r.src=S,r.controls=!0,r.autoplay=!0,s||(r.setAttribute("controlsList","nodownload"),r.oncontextmenu=i=>i.preventDefault()),a.appendChild(r)}else if(e.type==="application/pdf"){const r=document.createElement("iframe");r.src=S+(s?"":"#toolbar=0"),r.style.width="100%",r.style.height="100%",a.appendChild(r)}else e.type==="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"||e.type==="application/vnd.ms-excel"||e.name.endsWith(".xlsx")||e.name.endsWith(".xls")||e.name.endsWith(".csv")?rt(n,a):e.type==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"||e.name.endsWith(".docx")?await at(n,a):a.innerText="Preview not supported for this file type.";document.getElementById("viewer-filename").innerText=e.name,Te.classList.remove("hidden"),bt(e),document.getElementById("viewer-timer").classList.add("hidden")}catch(n){console.error(n),await f("Error","Decryption failed.")}}async function de(){Te.classList.add("hidden"),R&&R.classList.add("hidden"),document.getElementById("viewer-content").innerHTML="",S&&(URL.revokeObjectURL(S),S=null)}async function Dt(){localStorage.getItem("sv_crash_guard")&&(localStorage.removeItem("sv_crash_guard"),B())}async function B(e=""){ne.innerHTML="";let t=await E.getAllFiles();M=t,dt(t),e&&(t=t.filter(a=>a.name.toLowerCase().includes(e.toLowerCase())));const n=Se?.value||"date-desc";t.sort((a,s)=>{if(a.favorite&&!s.favorite)return-1;if(!a.favorite&&s.favorite)return 1;switch(n){case"date-desc":return(s.date||0)-(a.date||0);case"date-asc":return(a.date||0)-(s.date||0);case"name-asc":return a.name.localeCompare(s.name);case"name-desc":return s.name.localeCompare(a.name);case"size-desc":return(s.size||0)-(a.size||0);case"size-asc":return(a.size||0)-(s.size||0);default:return 0}});const o=document.getElementById("quick-tips");if(o&&(M.length===0?o.classList.remove("hidden"):o.classList.add("hidden")),t.length===0){ne.innerHTML=`
      <div class="empty-state cyber-empty-state">
        ...
        <div class="file-card-actions">
          ...
          <button class="btn-highlight custom-share-btn">${L} Custom</button>
    ...
    `,r.querySelector(".select-checkbox").onchange=m=>{m.stopPropagation(),m.target.checked?b.add(a.id):b.delete(a.id),_()},r.querySelector(".open-btn").onclick=m=>{m.stopPropagation(),le(a.id)},r.querySelector(".download-btn").onclick=m=>Bt(m,a.id),r.querySelector(".info-btn").onclick=m=>It(m,a.id),r.querySelector(".rename-btn").onclick=m=>kt(m,a.id,a.name),r.querySelector(".share-btn").onclick=m=>Lt(m,a.id),r.querySelector(".custom-share-btn").onclick=m=>Ct(m,a.id),r.querySelector(".delete-btn").onclick=m=>xt(m,a.id),ne.appendChild(r)})}function pe(){const e=localStorage.getItem("sv_panic_action")||"blur",t=document.getElementById("panic-action-select");t&&(t.value=e);const n=localStorage.getItem("sv_dl_media")!=="false",o=localStorage.getItem("sv_dl_doc")!=="false",a=document.getElementById("dl-media-toggle"),s=document.getElementById("dl-doc-toggle");a&&(a.checked=n),s&&(s.checked=o),ce();const r=localStorage.getItem("sv_panic_enabled")!=="false",i=document.getElementById("panic-enable-toggle");i&&(i.checked=r),fe()}function fe(){const e=localStorage.getItem("sv_panic_enabled")!=="false",t=document.getElementById("header-panic-wrapper"),n=document.getElementById("panic-settings-content");t&&(e?(t.classList.remove("hidden"),t.style.display="flex"):(t.classList.add("hidden"),t.style.display="none")),n&&(e?(n.classList.remove("hidden"),n.style.opacity="1",n.style.pointerEvents="auto"):(n.classList.add("hidden"),n.style.opacity="0.5",n.style.pointerEvents="none"))}function me(){const e=localStorage.getItem("sv_panic_action")||"blur";if(e!=="none"){if(e==="erase")(async()=>{try{const t=localStorage.getItem("sv_panic_action"),n=await E.getAllFiles();for(const o of n)await E.deleteFile(o.id);localStorage.clear(),t&&localStorage.setItem("sv_panic_action",t),window.location.reload()}catch(t){console.error(t)}})();else if(e==="blur"||e==="lock"){de(),K?.close(),Z?.close(),document.querySelectorAll(".panic-overlay").forEach(n=>n.remove());const t=document.createElement("div");t.className="panic-overlay blur-mode",t.innerHTML=`
    ...
    `,document.body.appendChild(t);const n=()=>{t.remove()};t.addEventListener("click",n),t.addEventListener("touchstart",n)}}}function Mt(){const e=document.getElementById("cyber-matrix-canvas");if(e){const o=e.getContext("2d");if(o){let p=function(){a=e.width=window.innerWidth,s=e.height=window.innerHeight,l=Math.floor(a/c),d=[];for(let h=0;h<l;h++)d[h]=Math.random()*-50},I=function(h){if(requestAnimationFrame(I),!(h-y<L)){y=h,o.fillStyle="rgba(5, 7, 10, 0.14)",o.fillRect(0,0,a,s),o.font=`${c}px "JetBrains Mono", monospace`;for(let u=0;u<d.length;u++){const m=i[Math.floor(Math.random()*i.length)],k=u*c,T=d[u]*c;Math.random()>.9?(o.fillStyle="#00f0ff",o.shadowColor="#00f0ff",o.shadowBlur=6):(o.fillStyle=u%2===0?"rgba(0, 240, 255, 0.45)":"rgba(0, 255, 136, 0.4)",o.shadowBlur=0),o.fillText(m,k,T),T>s&&Math.random()>.975&&(d[u]=0),d[u]++}}},a=e.width=window.innerWidth,s=e.height=window.innerHeight;const i="0123456789ABCDEF•:;><[]{}/*~$=+!#@%&_AES-256GCMKEYPBKDF2".split(""),c=13;let l=Math.floor(a/c),d=[];p(),window.addEventListener("resize",p);let y=0;const L=45;requestAnimationFrame(I)}}const t=document.getElementById("hero-term-status");if(t){const o=["INITIALIZING SECURE ENVIRONMENT... CRYPTOGRAPHIC ENGINE: READY [AES-GCM-256] • FILE PROCESSOR: READY • PROTECTION LAYER: ACTIVE","HARDWARE ACCELERATION: ACTIVE // ZERO-KNOWLEDGE WEB CRYPTO RUNTIME ONLINE","PBKDF2 KEY DERIVATION: SHA-256 WITH 600,000 ITERATIONS ENFORCED","LOCAL MEMORY SANDBOX: ISOLATED // 0 BYTES TRANSMITTED TO REMOTE NETWORKS","MIL-SPEC DEFENSE PROTOCOL: LEVEL-4 CLEARANCE // AUTHENTICATION TAG VERIFICATION ARMED","CONTAINER ENCRYPTION: 256-BIT CRYPTOGRAPHIC SEED + UNIQUE 128-BIT IV PER TRANSACTION","STANDALONE PACKAGER: READY // OFFLINE ZERO-DEPENDENCY DECRYPTOR COMPLIANT","SYSTEM HEALTH: OPTIMAL // LOCAL STORAGE VAULT INTEGRITY 100% VERIFIED"];let a=0;setInterval(()=>{a=(a+1)%o.length,t.style.opacity="0",t.style.transform="translateY(4px)",setTimeout(()=>{t.textContent=o[a],t.style.transition="all 0.3s ease",t.style.opacity="1",t.style.transform="translateY(0)"},300)},4500)}const n=[{el:document.getElementById("hero-command"),navId:"nav-item-console"},{el:document.getElementById("inline-add-container"),navId:"nav-item-protect"},{el:document.getElementById("secured-files-heading"),navId:"nav-item-files"}];window.addEventListener("scroll",()=>{const o=window.scrollY+180;for(let a=n.length-1;a>=0;a--){const s=n[a];if(s.el&&s.el.offsetTop<=o){document.querySelectorAll(".cyber-nav-item").forEach(r=>{r.id===s.navId?r.classList.add("active"):r.id!=="nav-item-settings"&&r.classList.remove("active")});break}}},{passive:!0})}
