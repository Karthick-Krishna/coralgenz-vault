import"./jspdf-SUxvxRxM.js";import{r as He,u as We}from"./xlsx-DFH0qU2H.js";import{r as $e}from"./mammoth-PVFyTYmD.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&o(r)}).observe(document,{childList:!0,subtree:!0});function n(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(a){if(a.ep)return;a.ep=!0;const i=n(a);fetch(a.href,i)}})();const Z="CORALGENZ::MILSPEC_V5::ANTI_OFFLINE_CRACKER::ZERO_KNOWLEDGE::774910283419",Ve="CORALGENZ::MILSPEC_V4::ANTI_JOHN_THE_RIPPER::ZERO_KNOWLEDGE::992174829104";class y{constructor(){this.algo={name:"AES-GCM",length:256}}static generateIV(){return window.crypto.getRandomValues(new Uint8Array(12))}static generateSalt(){return window.crypto.getRandomValues(new Uint8Array(32))}static async generateKey(){return window.crypto.subtle.generateKey({name:"AES-GCM",length:256},!0,["encrypt","decrypt","wrapKey","unwrapKey"])}static async deriveKeyFromPassword(t,n,o=2e6,a="SHA-256",i=!0,r=Z){const s=new TextEncoder;let c;if(i){const u=r||Z,g=s.encode(u),I=s.encode(t),b=new Uint8Array(g.length+I.length);b.set(g,0),b.set(I,g.length);const w=await window.crypto.subtle.importKey("raw",n,{name:"HMAC",hash:"SHA-512"},!1,["sign"]),m=await window.crypto.subtle.sign("HMAC",w,b);c=new Uint8Array(m),b.fill(0),I.fill(0)}else c=s.encode(t);const l=await window.crypto.subtle.importKey("raw",c,"PBKDF2",!1,["deriveKey"]),d=await window.crypto.subtle.deriveKey({name:"PBKDF2",salt:n,iterations:o,hash:a},l,{name:"AES-GCM",length:256},!0,["encrypt","decrypt","wrapKey","unwrapKey"]);return c&&c.fill&&c.fill(0),d}static async encryptData(t,n){const o=this.generateIV(),a=await window.crypto.subtle.encrypt({name:"AES-GCM",iv:o},t,n);return{iv:o,ciphertext:a}}static async decryptData(t,n,o){return window.crypto.subtle.decrypt({name:"AES-GCM",iv:n},t,o)}static async exportKey(t){return window.crypto.subtle.exportKey("raw",t)}static async importKey(t){return window.crypto.subtle.importKey("raw",t,{name:"AES-GCM"},!0,["encrypt","decrypt","wrapKey","unwrapKey"])}static async wrapKey(t,n){const o=this.generateIV(),a=await window.crypto.subtle.wrapKey("raw",t,n,{name:"AES-GCM",iv:o});return{iv:o,wrappedData:a}}static async unwrapKey(t,n,o){return window.crypto.subtle.unwrapKey("raw",t,n,{name:"AES-GCM",iv:o},{name:"AES-GCM"},!0,["encrypt","decrypt"])}static async unwrapWithFallback(t,n,o,a,i=2e6){try{const r=await this.deriveKeyFromPassword(n,o,i,"SHA-256",!0,Z);return await this.unwrapKey(t,r,a)}catch(r){try{const s=await this.deriveKeyFromPassword(n,o,1e6,"SHA-256",!0,Ve);return await this.unwrapKey(t,s,a)}catch{try{const c=await this.deriveKeyFromPassword(n,o,1e6,"SHA-256",!0,Z);return await this.unwrapKey(t,c,a)}catch{const l=[i,1e6,6e5,1e5];for(const d of l)try{const u=await this.deriveKeyFromPassword(n,o,d,"SHA-256",!1);return await this.unwrapKey(t,u,a)}catch{}throw r}}}}static async getDeviceKey(){const t=localStorage.getItem("sv_device_key");let n;if(t){const o=atob(t),a=new Uint8Array(o.length);for(let i=0;i<o.length;i++)a[i]=o.charCodeAt(i);n=a.buffer}else{const o=await this.generateKey();n=await this.exportKey(o);const a=new Uint8Array(n);let i="";for(let r=0;r<a.byteLength;r++)i+=String.fromCharCode(a[r]);localStorage.setItem("sv_device_key",btoa(i))}return this.importKey(n)}}const Ge="SecureVaultDB",Ye=1,C="files";function J(){return new Promise((e,t)=>{const n=indexedDB.open(Ge,Ye);n.onupgradeneeded=o=>{const a=o.target.result;a.objectStoreNames.contains(C)||a.createObjectStore(C,{keyPath:"id"})},n.onsuccess=o=>e(o.target.result),n.onerror=o=>t(o.target.error)})}const E={async saveFile(e){const t=await J();return new Promise((n,o)=>{const r=t.transaction(C,"readwrite").objectStore(C).put(e);r.onsuccess=()=>n(e.id),r.onerror=()=>o(r.error)})},async getAllFiles(){const e=await J();return new Promise((t,n)=>{const i=e.transaction(C,"readonly").objectStore(C).openCursor(),r=[];i.onsuccess=s=>{const c=s.target.result;if(c){const{content:l,...d}=c.value;r.push(d),c.continue()}else t(r)},i.onerror=()=>n(i.error)})},async getFile(e){const t=await J();return new Promise((n,o)=>{const r=t.transaction(C,"readonly").objectStore(C).get(e);r.onsuccess=()=>n(r.result),r.onerror=()=>o(r.error)})},async deleteFile(e){const t=await J();return new Promise((n,o)=>{const r=t.transaction(C,"readwrite").objectStore(C).delete(e);r.onsuccess=()=>n(),r.onerror=()=>o(r.error)})},async updateFile(e){return this.saveFile(e)}};var je=$e();const oe=document.getElementById("file-list");document.getElementById("add-file-btn");const $=document.getElementById("auth-modal"),Me=document.getElementById("viewer"),F=document.getElementById("viewer-download-btn"),M=document.getElementById("file-input");document.getElementById("privacy-curtain");const qe=document.getElementById("theme-toggle"),H=document.getElementById("search-input"),Pe=document.getElementById("sort-select"),Ze=document.getElementById("storage-text"),Je=document.getElementById("storage-fill"),xe=document.getElementById("bulk-actions"),Xe=document.getElementById("selected-count"),Qe=document.getElementById("bulk-delete-btn"),et=document.getElementById("cancel-select-btn"),ge=document.getElementById("info-modal"),te=document.getElementById("rename-modal"),U=document.getElementById("strength-bar"),W=document.getElementById("strength-text"),tt=document.getElementById("bulk-export-btn"),re=document.getElementById("recent-section"),ae=document.getElementById("recent-scroll"),X=document.getElementById("drop-zone"),Ie=document.getElementById("stat-total"),ke=document.getElementById("stat-images"),Be=document.getElementById("stat-videos"),Le=document.getElementById("stat-size"),z=document.getElementById("settings-modal"),Oe=document.getElementById("change-pass-modal"),nt=document.getElementById("settings-btn"),ot=document.getElementById("help-btn"),ee=document.getElementById("help-modal");let fe=null,A=null,P=[],x=new Set,Q=null,N=JSON.parse(localStorage.getItem("sv_recent")||"[]"),rt=null,se=null,S={},D=null;function V(e,t="",n={}){return new Promise(o=>{const a=document.getElementById("custom-prompt-modal"),i=document.getElementById("prompt-title"),r=document.getElementById("prompt-message"),s=document.getElementById("prompt-input"),c=document.getElementById("prompt-confirm"),l=document.getElementById("prompt-cancel");i.textContent=e,r.textContent=t,s.type=n.inputType||"text",s.placeholder=n.placeholder||"Enter value...",s.value="";const d=()=>{a.close(),c.onclick=null,l.onclick=null,s.onkeydown=null};c.onclick=()=>{const u=s.value;d(),o(u||null)},l.onclick=()=>{d(),o(null)},s.onkeydown=u=>{u.key==="Enter"&&(u.preventDefault(),c.click())},a.showModal(),setTimeout(()=>s.focus(),100)})}function Ne(e,t=""){return new Promise(n=>{const o=document.getElementById("custom-confirm-modal"),a=document.getElementById("confirm-title"),i=document.getElementById("confirm-message"),r=document.getElementById("confirm-ok"),s=document.getElementById("confirm-cancel");a.textContent=e,i.textContent=t;let c=!1;const l=()=>{o.close(),r.onclick=null,s.onclick=null,o.removeEventListener("click",d),o.removeEventListener("close",u)},d=g=>{g.target===o&&!c&&(c=!0,l(),n(!1))},u=()=>{c||(c=!0,l(),n(!1))};r.onclick=()=>{c||(c=!0,l(),n(!0))},s.onclick=()=>{c||(c=!0,l(),n(!1))},o.addEventListener("click",d),o.addEventListener("close",u),o.showModal()})}function f(e,t=""){return new Promise(n=>{const o=document.getElementById("custom-alert-modal"),a=document.getElementById("alert-title"),i=document.getElementById("alert-message"),r=document.getElementById("alert-ok");a.textContent=e,i.textContent=t;const s=l=>{l.key==="Enter"&&(l.preventDefault(),c(),n())},c=()=>{o.close(),r.onclick=null,document.removeEventListener("keydown",s)};r.onclick=()=>{c(),n()},document.addEventListener("keydown",s),o.showModal()})}async function at(){localStorage.removeItem("sv_app_lock"),localStorage.removeItem("sv_autolock"),localStorage.removeItem("sv_recovery_q"),localStorage.removeItem("sv_recovery_a"),ut(),ue(),await L(),st(),Ut(),ze(),he(),zt()}at();function h(e){const t=document.getElementById("security-terminal-body");if(t){const n=document.createElement("p");n.className="term-line",n.textContent=`> ${e}`,t.appendChild(n),t.scrollTop=t.scrollHeight}}function v(e,t="completed"){const n=document.getElementById(e);if(n){n.className=`process-step ${t}`.trim();const o=n.querySelector(".step-icon");o&&(t==="completed"?o.textContent="[✓]":t==="active"?o.textContent="[→]":o.textContent="[ ]")}}function R(e,t){const n=document.getElementById("protect-progress-bar"),o=document.getElementById("protect-progress-percent"),a=document.getElementById("protect-progress-label");n&&(n.style.width=`${e}%`),o&&(o.textContent=`${e}%`),a&&t&&(a.textContent=t)}function it(){const e="ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#$%^&*()_+",t=new Uint8Array(16);window.crypto.getRandomValues(t);let n="";for(let a=0;a<16;a++)n+=e[t[a]%e.length];const o=document.getElementById("new-password");if(o){o.value=n,o.type="text",Ke();const a=document.getElementById("toggle-new-password");a&&(a.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="23" x2="23" y2="1"></line></svg>',a.style.color="var(--cyber-cyan)")}}function st(){document.getElementById("cancel-add").addEventListener("click",()=>{_()}),document.getElementById("confirm-add").addEventListener("click",Ae),document.getElementById("generate-pwd-btn")?.addEventListener("click",it);const e=document.getElementById("toggle-new-password"),t=document.getElementById("new-password");e?.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),t&&(t.type==="password"?(t.type="text",e.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="23" x2="23" y2="1"></line></svg>',e.style.color="var(--cyber-cyan)"):(t.type="password",e.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>',e.style.color="var(--cyber-text-muted)"))}),t?.addEventListener("keydown",r=>{r.key==="Enter"&&(r.preventDefault(),Ae())}),document.getElementById("download-protected-html-btn")?.addEventListener("click",async()=>{if(!D||!D.record){f("Notice","No active protected file session. Please select a file from the vault below to download.");return}const r=document.getElementById("download-protected-html-btn"),s=r?r.innerHTML:"";try{r&&(r.disabled=!0,r.innerHTML=`
        ...
        `);let c=D.buffer,l=D.password;if(!c||c.byteLength===0){const d=await j(D.record,l);if(d&&d.buffer)c=d.buffer,l=d.password||l;else throw new Error("Could not retrieve file content for packaging.")}await Y(D.record,c,l,{},"Protected HTML package exported successfully!")}catch(c){console.error(c),await f("Export Error","Failed to download HTML package: "+c.message)}finally{r&&(r.disabled=!1,r.innerHTML=s)}}),document.getElementById("view-in-vault-btn")?.addEventListener("click",()=>{_(),document.getElementById("secured-files-heading")?.scrollIntoView({behavior:"smooth"})}),document.getElementById("protect-another-btn")?.addEventListener("click",()=>{_(),document.getElementById("inline-add-container")?.scrollIntoView({behavior:"smooth"})}),document.querySelectorAll(".cyber-nav-item").forEach(r=>{r.addEventListener("click",()=>{document.querySelectorAll(".cyber-nav-item").forEach(s=>s.classList.remove("active")),r.classList.add("active"),r.id==="nav-item-protect"&&_()})}),document.getElementById("sidebar-settings-btn")?.addEventListener("click",()=>{ue(),z?.showModal()}),document.getElementById("cancel-auth").addEventListener("click",()=>{$.close(),document.getElementById("auth-password").value="",fe=null}),document.getElementById("confirm-auth").addEventListener("click",De),document.getElementById("auth-password")?.addEventListener("keydown",r=>{r.key==="Enter"&&(r.preventDefault(),De())}),document.getElementById("close-viewer").addEventListener("click",pe),qe?.addEventListener("click",Fe),H?.addEventListener("input",mt),Pe?.addEventListener("change",gt),document.getElementById("new-password")?.addEventListener("input",Ke),Qe?.addEventListener("click",wt),tt?.addEventListener("click",vt),et?.addEventListener("click",ht),document.getElementById("select-all-btn")?.addEventListener("click",yt),document.getElementById("close-info")?.addEventListener("click",()=>ge.close()),document.getElementById("cancel-rename")?.addEventListener("click",()=>te.close()),document.getElementById("confirm-rename")?.addEventListener("click",Ce),document.getElementById("rename-input")?.addEventListener("keydown",r=>{r.key==="Enter"&&(r.preventDefault(),Ce())}),bt(),It(),ot?.addEventListener("click",()=>ee?.showModal()),document.getElementById("close-help")?.addEventListener("click",()=>ee?.close()),pt(),nt?.addEventListener("click",()=>{ue(),z?.showModal()}),document.getElementById("close-settings")?.addEventListener("click",()=>z?.close()),document.getElementById("close-settings-x")?.addEventListener("click",()=>z?.close()),document.getElementById("feedback-btn")?.addEventListener("click",()=>{const r="coralgenz@zohomail.in",s=encodeURIComponent("SecureVault Feedback"),c=encodeURIComponent(`Hi team,

I have some feedback for SecureVault:
`);window.location.href=`mailto:${r}?subject=${s}&body=${c}`}),document.querySelectorAll(".panic-trigger").forEach(r=>{r.addEventListener("click",me)}),document.getElementById("panic-btn")?.addEventListener("click",me),document.getElementById("panic-action-select")?.addEventListener("change",r=>{localStorage.setItem("sv_panic_action",r.target.value)}),document.getElementById("panic-enable-toggle")?.addEventListener("change",r=>{const s=r.target.checked;localStorage.setItem("sv_panic_enabled",s),he()}),document.getElementById("dl-media-toggle")?.addEventListener("change",r=>{localStorage.setItem("sv_dl_media",r.target.checked),le()}),document.getElementById("dl-doc-toggle")?.addEventListener("change",r=>{localStorage.setItem("sv_dl_doc",r.target.checked),le()}),document.getElementById("cancel-change-pass")?.addEventListener("click",()=>Oe?.close()),document.getElementById("confirm-change-pass")?.addEventListener("click",Tt),ie("toggle-new-password","new-password"),ie("toggle-auth-password","auth-password"),ie("toggle-share-password","share-password");const n=document.getElementById("share-modal");document.getElementById("cancel-share")?.addEventListener("click",()=>n?.close()),document.getElementById("confirm-share")?.addEventListener("click",Se),document.getElementById("share-password")?.addEventListener("keydown",r=>{r.key==="Enter"&&(r.preventDefault(),Se())}),document.getElementById("share-logo-zone")?.addEventListener("click",r=>{r.target!==document.getElementById("share-logo")&&document.getElementById("share-logo")?.click()}),document.getElementById("share-logo")?.addEventListener("change",r=>{const s=r.target.files[0],c=document.getElementById("share-logo-text");c&&(c.textContent=s?s.name:"Choose File")}),M?.addEventListener("change",Re),document.getElementById("file-remove-btn")?.addEventListener("click",ct);const o=document.getElementById("file-upload-zone"),a=document.getElementById("drop-text-primary"),i=document.getElementById("drop-text-secondary");o?.addEventListener("click",r=>{r.target!==M&&M?.click()}),o?.addEventListener("dragover",r=>{r.preventDefault(),o.classList.add("drag-over"),a&&(a.textContent="FILE DETECTED — INITIALIZING PROTECTION"),i&&(i.textContent="Release file to initialize security inspection")}),o?.addEventListener("dragleave",()=>{o.classList.remove("drag-over"),a&&(a.textContent="DROP FILE TO SECURE"),i&&(i.textContent="or click to browse local file system")}),o?.addEventListener("drop",()=>{o.classList.remove("drag-over"),a&&(a.textContent="DROP FILE TO SECURE"),i&&(i.textContent="or click to browse local file system")}),document.addEventListener("contextmenu",r=>r.preventDefault()),document.addEventListener("keydown",r=>{(r.key==="PrintScreen"||r.ctrlKey&&r.key==="p")&&(r.preventDefault(),alert("Screenshots are disabled"))})}function ie(e,t){const n=document.getElementById(e),o=document.getElementById(t);!n||!o||n.addEventListener("click",a=>{a.preventDefault(),o.type==="password"?(o.type="text",n.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="23" x2="23" y2="1"></line></svg>',n.style.color="var(--cyber-cyan)"):(o.type="password",n.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>',n.style.color="var(--cyber-muted)")})}function _(){M.value="",document.getElementById("new-password").value="",D=null,U.className="strength-bar",W.innerText="ENTER PASSWORD";const e=document.getElementById("file-upload-zone"),t=document.getElementById("file-preview"),n=document.getElementById("inline-password-section"),o=document.getElementById("protection-process-container"),a=document.getElementById("protection-complete-container");e?.classList.remove("hidden"),t?.classList.add("hidden"),n?.classList.add("hidden"),o?.classList.add("hidden"),a?.classList.add("hidden");const i=document.getElementById("process-timer-display");i&&(i.textContent="00:00.00");const r=document.querySelector(".scanner-center-shield");r&&r.classList.remove("success");const s=document.getElementById("scanner-status-text");s&&(s.textContent="SECURING");const c=document.getElementById("security-terminal-body");c&&(c.innerHTML='<p class="term-line">&gt; Standby for cryptographic instruction...</p>'),["step-analysis","step-prep","step-kdf","step-encrypt","step-meta","step-finalize","step-output","step-verify"].forEach(l=>{v(l,"")}),R(0,"STANDBY")}function Re(){const e=M.files[0];if(!e)return;const t=150*1024*1024;if(e.size>t){f("File Too Large","Please select a file smaller than 150 MB."),M.value="";return}const n=document.getElementById("file-upload-zone"),o=document.getElementById("file-preview"),a=document.getElementById("file-preview-name"),i=document.getElementById("file-preview-size"),r=document.getElementById("protection-process-container"),s=document.getElementById("protection-complete-container");a&&(a.textContent=e.name),i&&(i.textContent=ce(e.size)),n?.classList.add("hidden"),r?.classList.add("hidden"),s?.classList.add("hidden"),o?.classList.remove("hidden"),document.getElementById("inline-password-section")?.classList.remove("hidden")}function ct(e){e.preventDefault(),e.stopPropagation(),M.value="",D=null;const t=document.getElementById("file-upload-zone"),n=document.getElementById("file-preview"),o=document.getElementById("inline-password-section"),a=document.getElementById("protection-process-container"),i=document.getElementById("protection-complete-container");t?.classList.remove("hidden"),n?.classList.add("hidden"),o?.classList.add("hidden"),a?.classList.add("hidden"),i?.classList.add("hidden"),t?.classList.remove("hidden"),n?.classList.add("hidden"),document.getElementById("inline-password-section")?.classList.add("hidden")}function ce(e){if(e===0)return"0 Bytes";const t=1024,n=["Bytes","KB","MB","GB"],o=Math.floor(Math.log(e)/Math.log(t));return parseFloat((e/Math.pow(t,o)).toFixed(2))+" "+n[o]}function lt(e,t){try{const n=He(new Uint8Array(e),{type:"array"}),o=n.SheetNames;if(o.length===0)return;const a=document.createElement("div");a.className="excel-viewer";const i=document.createElement("div");i.className="excel-header";const r=document.createElement("div");r.className="excel-table-wrapper";let s=o[0];const c=l=>{r.innerHTML="";const d=n.Sheets[l],u=We.sheet_to_json(d,{header:1,defval:""});if(!u||u.length===0){r.innerHTML='<div style="padding:20px;text-align:center;">Empty Sheet</div>';return}const g=document.createElement("table");g.className="excel-table",u.forEach((I,b)=>{const w=document.createElement("tr");I.forEach(m=>{const p=b===0?"th":"td",B=document.createElement(p);B.textContent=m!==void 0?m:"",w.appendChild(B)}),g.appendChild(w)}),r.appendChild(g)};o.forEach(l=>{const d=document.createElement("button");d.className=`excel-sheet-btn ${l===s?"active":""}`,d.textContent=l,d.onclick=()=>{s=l,c(l),i.querySelectorAll(".excel-sheet-btn").forEach(u=>u.classList.remove("active")),d.classList.add("active")},i.appendChild(d)}),c(s),a.appendChild(i),a.appendChild(r),t.appendChild(a)}catch(n){console.error(n),t.innerHTML=`<div style="padding:20px;color:red;">Error parsing Excel file: ${n.message}</div>`}}async function dt(e,t){try{const{value:n,messages:o}=await je.convertToHtml({arrayBuffer:e}),a=document.createElement("div");a.className="word-viewer";const i=document.createElement("div");i.className="word-document",i.innerHTML=n,a.appendChild(i),t.appendChild(a)}catch(n){console.error(n),t.innerHTML=`<div style="padding:20px;color:red;">Error parsing Word file: ${n.message}</div>`}}function pt(){document.querySelectorAll("dialog.modal").forEach(t=>{t.addEventListener("click",n=>{const o=t.getBoundingClientRect();n.clientX>=o.left&&n.clientX<=o.right&&n.clientY>=o.top&&n.clientY<=o.bottom,n.target===t&&(t.close(),t.id==="add-modal"&&_())})})}function ut(){localStorage.getItem("sv_theme")==="light"&&document.body.classList.add("light-theme")}function Fe(){document.body.classList.toggle("light-theme");const e=document.body.classList.contains("light-theme");localStorage.setItem("sv_theme",e?"light":"dark")}function Ke(){const e=document.getElementById("new-password").value;let t=0;e.length>=6&&t++,e.length>=10&&t++,/[A-Z]/.test(e)&&/[a-z]/.test(e)&&t++,/[0-9]/.test(e)&&t++,/[^A-Za-z0-9]/.test(e)&&t++,U.className="strength-bar",t<=1?(U.classList.add("weak"),W.innerText="Weak"):t===2?(U.classList.add("fair"),W.innerText="Fair"):t===3?(U.classList.add("good"),W.innerText="Good"):(U.classList.add("strong"),W.innerText="Strong 💪")}function mt(){const e=H.value.toLowerCase().trim();L(e)}function gt(){L(H?.value||"")}function ft(e){const t=e.reduce((r,s)=>r+(s.size||0),0),n=(t/1024/1024).toFixed(2);Ze.innerText=`${e.length} files • ${n} MB used`;const o=Math.min(t/(500*1024*1024)*100,100);Je.style.width=o+"%";const a=e.filter(r=>r.type?.startsWith("image")).length,i=e.filter(r=>r.type?.startsWith("video")).length;Ie&&(Ie.innerText=e.length),ke&&(ke.innerText=a),Be&&(Be.innerText=i),Le&&(Le.innerText=n)}function G(){const e=document.getElementById("select-all-btn");x.size>0?(xe.classList.remove("hidden"),Xe.innerText=`${x.size} selected`,e&&(x.size>=P.length&&P.length>0?e.innerText="Deselect All":e.innerText="Select All")):xe.classList.add("hidden")}function yt(){x.size>=P.length&&P.length>0?x.clear():P.forEach(e=>x.add(e.id)),G(),L(H?.value||"")}function ht(){x.clear(),G(),L(H?.value||"")}async function wt(){if(await Ne("Delete Files",`Delete ${x.size} file(s)? This cannot be undone.`)){for(const t of x)await E.deleteFile(t);x.clear(),G(),L(),await f("Success","Files deleted.")}}async function vt(){if(x.size!==0){await f("Export",`Exporting ${x.size} files. Each will download separately.`);for(const e of x)await E.getFile(e)&&await Et(e);x.clear(),G(),L()}}async function Et(e){const t=await E.getFile(e);if(!t)return;const n=await j(t);if(!n||!n.buffer)return;const{buffer:o,password:a}=n;let i=a;i||(i=await V("Export Protected File","Enter the file's password to protect this export:",{inputType:"password",placeholder:"Enter original password..."})),i&&await Y(t,o,i,{},"Protected file downloaded successfully!")}function bt(){const e=document.getElementById("app");["dragenter","dragover"].forEach(t=>{e?.addEventListener(t,n=>{n.preventDefault(),X?.classList.remove("hidden"),X?.classList.add("active")})}),["dragleave","drop"].forEach(t=>{e?.addEventListener(t,n=>{n.preventDefault(),X?.classList.add("hidden"),X?.classList.remove("active")})}),e?.addEventListener("drop",async t=>{t.preventDefault();const n=t.dataTransfer?.files;if(n&&n.length>0)if(n.length===1){const o=new DataTransfer;o.items.add(n[0]),M.files=o.files,Re(),document.getElementById("inline-add-container")?.scrollIntoView({behavior:"smooth"})}else for(const o of n)await xt(o)})}async function xt(e){if(e.size>157286400){await f("File Too Large",`File "${e.name}" is too large. Max 150 MB.`);return}const n=await V("Set Password",`Set password for: ${e.name}`,{inputType:"password",placeholder:"Enter secure password..."});if(!n)return;const o=await y.generateKey(),a=y.generateSalt(),i=await y.deriveKeyFromPassword(n,a,2e6),r=await e.arrayBuffer(),{iv:s,ciphertext:c}=await y.encryptData(o,r),{iv:l,wrappedData:d}=await y.wrapKey(o,i),u={id:crypto.randomUUID(),name:e.name,type:e.type,size:e.size,date:Date.now(),authMode:"always",keys:[{type:"password",salt:a,iv:l,data:d}],content:c,iv:s,viewCount:0};await E.saveFile(u),L(),await f("Success",`${e.name} encrypted and saved!`)}function It(){document.addEventListener("keydown",e=>{(e.ctrlKey||e.metaKey)&&e.key==="n"&&e.preventDefault(),(e.ctrlKey||e.metaKey)&&e.key==="f"&&(e.preventDefault(),H?.focus()),e.key==="Escape"&&(_(),$.close(),ge?.close(),te?.close()),(e.ctrlKey||e.metaKey)&&e.shiftKey&&e.key.toLowerCase()==="p"&&(e.preventDefault(),me()),(e.ctrlKey||e.metaKey)&&e.key==="t"&&(e.preventDefault(),Fe())})}const Te=5,kt=300*1e3;function Ue(e){const t=S[e];if(t&&t.locked&&Date.now()<t.lockedUntil){const n=Math.ceil((t.lockedUntil-Date.now())/1e3);return alert(`File locked. Try again in ${n} seconds.`),!0}return!1}function Bt(e){S[e]||(S[e]={count:0,locked:!1,lockedUntil:0}),S[e].count++;const t=Te-S[e].count,n=document.getElementById("attempts-left"),o=document.getElementById("auth-attempts");t<=3&&(o?.classList.remove("hidden"),n&&(n.innerText=t)),S[e].count>=Te&&(S[e].locked=!0,S[e].lockedUntil=Date.now()+kt,$.close(),alert("Too many failed attempts. File locked for 5 minutes."))}function Lt(e){delete S[e],document.getElementById("auth-attempts")?.classList.add("hidden")}async function Tt(){const e=document.getElementById("current-password").value,t=document.getElementById("new-password-change").value,n=document.getElementById("confirm-password-change").value;if(!e||!t||!n){await f("Required","Please fill all fields");return}if(t!==n){await f("Error","New passwords do not match");return}if(t.length<4){await f("Error","New password must be at least 4 characters");return}try{const o=await E.getFile(rt);if(!o)throw new Error("File not found");const a=o.keys.find(d=>d.type==="password");if(!a)throw new Error("No password key found");const i=await y.unwrapWithFallback(a.data,e,a.salt,a.iv),r=y.generateSalt(),s=await y.deriveKeyFromPassword(t,r,2e6),{iv:c,wrappedData:l}=await y.wrapKey(i,s);o.keys=o.keys.filter(d=>d.type!=="password"),o.keys.push({type:"password",salt:r,iv:c,data:l}),o.accessLog=o.accessLog||[],o.accessLog.push({action:"password_changed",date:Date.now()}),await E.updateFile(o),Oe?.close(),await f("Success","Password changed successfully!")}catch(o){console.error(o),await f("Error","Failed to change password. Current password may be incorrect.")}}function Ct(e,t){e.accessLog=e.accessLog||[],e.accessLog.push({action:t,date:Date.now()}),e.accessLog.length>20&&(e.accessLog=e.accessLog.slice(-20))}function St(e){N=N.filter(t=>t.id!==e.id),N.unshift({id:e.id,name:e.name,type:e.type,date:Date.now()}),N.length>5&&(N=N.slice(0,5)),localStorage.setItem("sv_recent",JSON.stringify(N)),ze()}function ze(){if(!re||!ae)return;const e=N.filter(t=>P.some(n=>n.id===t.id));if(e.length===0){re.classList.add("hidden");return}re.classList.remove("hidden"),ae.innerHTML=e.map(t=>`
    <div class="recent-item" data-id="${t.id}">
      <span>${t.type?.startsWith("image")?"🖼️":t.type?.startsWith("video")?"🎬":"📄"}</span>
      <span>${t.name}</span>
    </div>
  `).join(""),ae.querySelectorAll(".recent-item").forEach(t=>{t.onclick=()=>de(t.dataset.id)})}async function At(e,t){e.stopPropagation(),await Ne("Delete File","Delete this file permanently?")&&(await E.deleteFile(t),L())}async function Dt(e,t){e.stopPropagation();const n=P.find(i=>i.id===t);if(!n)return;document.getElementById("info-name").innerText=n.name,document.getElementById("info-type").innerText=n.type||"Unknown",document.getElementById("info-size").innerText=(n.size/1024/1024).toFixed(2)+" MB",document.getElementById("info-date").innerText=new Date(n.date).toLocaleDateString(),document.getElementById("info-mode").innerText=n.authMode==="always"?"Always Ask Password":n.authMode||"Always Ask Password";const o=document.getElementById("info-views");o&&(o.innerText=n.viewCount||0);const a=document.getElementById("access-log");a&&n.accessLog&&n.accessLog.length>0?a.innerHTML=n.accessLog.slice(-10).reverse().map(i=>`
      <div class="access-log-item">
        ${i.action.replace("_"," ")} - ${new Date(i.date).toLocaleString()}
      </div>
    `).join(""):a&&(a.innerHTML="No access history"),ge.showModal()}function Mt(e,t,n){e.stopPropagation(),Q=t;const o=document.getElementById("rename-input");o.value=n,te.showModal(),setTimeout(()=>{o.focus(),o.select()},100)}async function Ce(){const e=document.getElementById("rename-input").value.trim();if(!e||!Q)return;const t=await E.getFile(Q);t&&(t.name=e,await E.updateFile(t)),te.close(),Q=null,L()}async function Pt(e,t){e&&e.stopPropagation();try{const n=await E.getFile(t);if(!n){await f("Error","File not found");return}const o=await j(n);if(!o||!o.buffer)return;const{buffer:a,password:i}=o;let r=i;if(r||(r=await V("Download Protected File","Enter the file's password to protect this download:",{inputType:"password",placeholder:"Enter original password..."})),!r)return;await Y(n,a,r,{},"Downloaded! The protected file has been saved."),Ct(n,"downloaded"),await E.updateFile(n)}catch(n){console.error(n),await f("Error","Failed to download file: "+(n.message||"Decryption error"))}}async function Ot(e,t){e.stopPropagation();try{const n=await E.getFile(t);if(!n)return;const o=await j(n);if(!o||!o.buffer)return;const{buffer:a,password:i}=o;let r=i;if(r||(r=await V("Share File","Enter the file's password to protect this export:",{inputType:"password",placeholder:"Enter original password..."})),!r)return;await Y(n,a,r,{})}catch(n){console.error(n),await f("Error","Share failed: "+n.message)}}function Nt(e,t){e.stopPropagation(),se=t,document.getElementById("share-password").value="",document.getElementById("share-title").value="",document.getElementById("share-logo").value="",document.getElementById("share-modal")?.showModal()}async function Se(){const e=document.getElementById("share-modal"),t=document.getElementById("share-password").value,n=document.getElementById("share-title").value||"SecureVault",o=document.getElementById("share-logo");if(!t){await f("Required","Please set a password for the file.");return}const a=document.getElementById("confirm-share"),i=a.innerText;a.innerText="Exporting...";try{const r=await E.getFile(se);if(!r)throw new Error("File not found");const s=await j(r,t);if(!s||!s.buffer)throw new Error("Decryption failed");const c=s.buffer;let l="";o.files&&o.files[0]&&(l=await new Promise(u=>{const g=new FileReader;g.onload=()=>u(g.result),g.readAsDataURL(o.files[0])})),await Y(r,c,t,{title:n,logoUrl:l}),e.close(),se=null}catch(r){console.error(r),await f("Error","Export failed: "+r.message)}finally{a.innerText=i}}function Rt(e,t){const n=(t||"").toLowerCase(),o=(e||"").toLowerCase();return n.startsWith("image/")||n.startsWith("video/")||n.startsWith("audio/")?!0:/\.(jpg|jpeg|png|gif|webp|svg|bmp|ico|mp4|webm|mov|mkv|ogg|mp3|wav|aac|m4a|flac)$/i.test(o)}function _e(){const e=localStorage.getItem("sv_dl_media")!=="false",t=localStorage.getItem("sv_dl_doc")!=="false";return{allowMedia:e,allowDoc:t}}function ye(e,t){const{allowMedia:n,allowDoc:o}=_e();return Rt(e,t)?n:o}function le(){const{allowMedia:e,allowDoc:t}=_e(),n=document.getElementById("dl-media-status-badge"),o=document.getElementById("dl-doc-status-badge");n&&(n.textContent=e?"ALLOWED":"RESTRICTED",n.className=`policy-status-badge ${e?"allowed":"restricted"}`),o&&(o.textContent=t?"ALLOWED":"RESTRICTED",o.className=`policy-status-badge ${t?"allowed":"restricted"}`)}async function Y(e,t,n,o={},a="Protected file downloaded successfully!"){if(!t||t.byteLength===0)throw new Error("No decrypted file payload available to package.");if(!n)throw new Error("Protection password is required to export this file.");const i=o.allowDownload!==void 0?!!o.allowDownload:ye(e.name,e.type),r=y.generateSalt(),s=await y.deriveKeyFromPassword(n,r,2e6),{iv:c,ciphertext:l}=await y.encryptData(s,t),u=await(B=>new Promise(O=>{const T=new FileReader;T.readAsDataURL(B),T.onloadend=()=>{const K=T.result||"";O(K.split(",")[1]||"")}}))(new Blob([l]));if(!u)throw new Error("Failed to serialize encrypted payload.");const{header:g,footer:I}=Ft(e,r,c,{...o,allowDownload:i}),b=new Blob([g,u,I],{type:"text/html;charset=utf-8"}),w=URL.createObjectURL(b),m=document.createElement("a");m.href=w;const p=e.name||"protected_file";m.download=p.endsWith(".secure.html")?p:p+".secure.html",document.body.appendChild(m),m.click(),document.body.removeChild(m),setTimeout(()=>URL.revokeObjectURL(w),6e4),a&&await f("Success",a)}function Ft(e,t,n,o={}){const a=ne=>btoa(String.fromCharCode(...new Uint8Array(ne))),i=a(t),r=a(n),s=o.title||"Coralgenz Vault",c=o.logoUrl||"",l=e.name||"Protected File",d=e.type||"application/octet-stream",u=Number(e.size)||0,g=e.id||"",I=o.allowDownload!==void 0?!!o.allowDownload:ye(l,d),b=JSON.stringify(i),w=JSON.stringify(r),m=JSON.stringify(d),p=JSON.stringify(l),B=JSON.stringify(u),O=JSON.stringify(s),T=JSON.stringify(g),K=JSON.stringify(I),q=c?`<img src="${c}" alt="Logo" class="brand-custom-logo">`:`<div class="brand-shield-icon">
...
<head>
    ...
    <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline' https://fonts.googleapis.com; font-src https://fonts.gstatic.com data:; img-src 'self' data: blob:; media-src 'self' blob:; frame-src blob:; script-src 'unsafe-inline'; connect-src 'none'; form-action 'none'; base-uri 'none'; object-src 'none';">
    <title>${s} // ${l}</title>
...
<body>
    ...
    <div id="auth-panel" class="auth-container">
        ...
        <div class="security-specs-chips">
            ...
            <span class="anti-cracker-chip">● ANTI-CRACKER PEPPER V5</span>
    ...
    <script>
        ...
        // Constant anti-cracker cryptographic pepper (Domain-separated V5 standard)
        const MILSPEC_PEPPER = "CORALGENZ::MILSPEC_V5::ANTI_OFFLINE_CRACKER::ZERO_KNOWLEDGE::774910283419";
        ...
        async function unlock() {
            ...
            try {
                ...
                // Helper to attempt dual-stage KDF
                async function tryDualStage(pepperStr, rounds) {
                ...
                // Tier 1: Current V5 Dual-Stage with 2,000,000 iterations (Anti-JohnTheRipper / Anti-Hashcat)
                try {
                    decrypted = await tryDualStage(MILSPEC_PEPPER, 2000000);
                } catch (t1Err) {
                    // Tier 2: Backward compatibility with V4 Dual-Stage (1,000,000 iterations)
                    try {
                        decrypted = await tryDualStage(MILSPEC_PEPPER_V4, 1000000);
                    } catch (t2Err) {
                        // Tier 3: Backward compatibility with legacy single-stage PBKDF2 (2M, 1M, 600k, 100k)
...
</html>`}}async function j(e,t=null){let n=null,o=t;if(t)try{const i=e.keys.find(r=>r.type==="password");i&&(n=await y.unwrapWithFallback(i.data,t,i.salt,i.iv))}catch{console.log("Provided password invalid for unlock")}if(!n){const i=await V("Decrypt for Export","Provide the ORIGINAL password to decrypt for export:",{inputType:"password",placeholder:"Enter password..."});if(!i)return null;try{const r=e.keys.find(s=>s.type==="password");n=await y.unwrapWithFallback(r.data,i,r.salt,r.iv),o=i}catch{return await f("Error","Incorrect password"),null}}return{buffer:await y.decryptData(n,e.iv,e.content),password:o}}async function Ae(){const e=M.files[0],t=document.getElementById("new-password").value;if(!e){await f("Required","Please select a file to protect");return}if(!t){await f("Required","Protection password is required");return}const n=document.getElementById("confirm-add"),o=n.innerText;n.innerText="SECURING PAYLOAD (5s)...",n.disabled=!0;const a=document.getElementById("protection-process-container"),i=document.getElementById("file-upload-zone"),r=document.getElementById("file-preview"),s=document.getElementById("inline-password-section"),c=document.getElementById("protection-complete-container"),l=document.getElementById("security-terminal-body"),d=document.getElementById("process-timer-display"),u=document.getElementById("scanner-status-text"),g=document.querySelector(".scanner-center-shield");i?.classList.add("hidden"),r?.classList.add("hidden"),s?.classList.add("hidden"),c?.classList.add("hidden"),a?.classList.remove("hidden"),["step-analysis","step-prep","step-kdf","step-encrypt","step-meta","step-finalize","step-output","step-verify"].forEach(m=>{v(m,"")}),g&&g.classList.remove("success"),u&&(u.textContent="SECURING"),l&&(l.innerHTML="");const I=Date.now(),b=5e3,w=setInterval(()=>{const m=Math.min(Date.now()-I,b),p=Math.floor(m/1e3).toString().padStart(2,"0"),B=Math.floor(m%1e3/10).toString().padStart(2,"0");d&&(d.textContent=`00:${p}.${B}`),m>=b&&clearInterval(w)},30);try{R(14,"FILE STRUCTURE ANALYSIS // MEMORY ENCLAVE..."),v("step-analysis","active"),h("[00:00.15] INITIATING ENCRYPTION PIPELINE // ZERO-KNOWLEDGE V2.6"),h(`[00:00.35] File: "${e.name}" [${ce(e.size)}] | Type: ${e.type||"application/octet-stream"}`),h(`[00:00.55] Enclave memory block allocated: ${e.size} bytes. Isolation confirmed.`),await new Promise(k=>setTimeout(k,700)),v("step-analysis","completed"),R(28,"INITIALIZING CSPRNG ENTROPY POOL..."),v("step-prep","active"),h("[00:00.85] Generating 256-bit cryptographic salt from hardware CSPRNG...");const m=y.generateSalt(),p=await y.generateKey();h("[00:01.10] Nonce generation: 96-bit AES-GCM Initialization Vector created."),h("[00:01.30] Ephemeral entropy validated: entropy score = 0.998."),await new Promise(k=>setTimeout(k,700)),v("step-prep","completed"),R(45,"DERIVING KEY (DUAL-STAGE KDF 2,000,000 ROUNDS)..."),v("step-kdf","active"),h("[00:01.50] Stage 1: HMAC-SHA512 Pre-whitening with Domain-Separated Pepper V5..."),h("[00:01.85] Stage 2: Computing 2,000,000 PBKDF2 iterations (Anti-JohnTheRipper / Anti-Hashcat)...");const B=await y.deriveKeyFromPassword(t,m,2e6);h("[00:02.15] Key derivation complete: 256-bit symmetric cipher key established."),await new Promise(k=>setTimeout(k,900)),v("step-kdf","completed"),R(65,"AES-GCM-256 CIPHER STREAM PROCESSING..."),v("step-encrypt","active"),h("[00:02.45] Executing client-side WebCrypto AES-GCM 256-bit cipher...");const O=await e.arrayBuffer(),T=O.slice(0),{iv:K,ciphertext:q}=await y.encryptData(p,O);h(`[00:02.85] Encrypting ${ce(e.size)} payload blocks into zero-knowledge ciphertext...`),h(`[00:03.15] Ciphertext generated (${q.byteLength} bytes). 128-bit Galois Tag verified.`),await new Promise(k=>setTimeout(k,1e3)),v("step-encrypt","completed"),R(80,"PACKAGING ZERO-KNOWLEDGE METADATA..."),v("step-meta","active"),v("step-finalize","active"),h("[00:03.40] Wrapping master file key with AES key wrap cipher...");const{iv:we,wrappedData:ve}=await y.wrapKey(p,B),ne=[{type:"password",salt:m,iv:we,data:ve}],Ee={id:crypto.randomUUID(),name:e.name,type:e.type,size:e.size,date:Date.now(),authMode:"always",keys:ne,content:q,iv:K,viewCount:0,expires:null,note:"",accessLog:[{action:"created",date:Date.now()}]};await E.saveFile(Ee),h("[00:03.75] Encrypted container committed to zero-knowledge local IndexedDB."),await new Promise(k=>setTimeout(k,700)),v("step-meta","completed"),v("step-finalize","completed"),R(92,"COMPILING STANDALONE HTML RUNTIME (.secure.html)..."),v("step-output","active"),h("[00:04.10] Assembling self-contained portable decryption engine..."),h("[00:04.35] Embedding browser-native WebCrypto decryptor payload..."),h("[00:04.55] Enforced security policy configured: ALWAYS ASK PASSWORD."),await new Promise(k=>setTimeout(k,650)),v("step-output","completed"),R(100,"CRYPTOGRAPHIC INTEGRITY VERIFIED [SEALED]"),v("step-verify","active"),h("[00:04.80] Authenticity check: GMAC integrity tag valid. Zero-knowledge verification OK."),h("[00:05.00] CRYPTOGRAPHIC CONTAINER LOCKED & SEALED. READY."),await new Promise(k=>setTimeout(k,350)),v("step-verify","completed"),clearInterval(w),d&&(d.textContent="00:05.00"),u&&(u.textContent="SECURED [✓]"),g&&g.classList.add("success"),await new Promise(k=>setTimeout(k,300)),a?.classList.add("hidden"),c?.classList.remove("hidden");const be=document.getElementById("complete-file-name");be&&(be.textContent=e.name),D={record:Ee,buffer:T,password:t},L()}catch(m){clearInterval(w),console.error(m),h(`CRITICAL ERROR: ${m.message}`),await f("Error","Encryption failed: "+m.message),a?.classList.add("hidden"),r?.classList.remove("hidden"),s?.classList.remove("hidden")}finally{n.innerText=o,n.disabled=!1}}async function de(e){if(!Ue(e))try{const t=await E.getFile(e);if(!t){await f("Error","File not found");return}fe=t,document.getElementById("auth-file-name").innerText=t.name,$.showModal()}catch(t){console.error(t),await f("Error","Error opening file")}}async function De(){const e=fe;if(!e||Ue(e.id))return;const t=document.getElementById("auth-password").value;if(!t)return;const n=document.getElementById("confirm-auth");n.innerText="Unlocking...";try{const o=e.keys.find(i=>i.type==="password");if(!o)throw new Error("Corrupt key data");const a=await y.unwrapWithFallback(o.data,t,o.salt,o.iv);Lt(e.id),$.close(),document.getElementById("auth-password").value="",Kt(e,a)}catch(o){console.error(o),Bt(e.id),await f("Error","Incorrect password or decryption error.")}finally{n.innerText="Unlock"}}async function Kt(e,t){try{const n=await y.decryptData(t,e.iv,e.content),o=new Blob([n],{type:e.type});A=URL.createObjectURL(o);const a=document.getElementById("viewer-content");a.innerHTML="";const i=ye(e.name,e.type);if(F&&(i?(F.classList.remove("hidden"),F.onclick=()=>{if(!A)return;const r=document.createElement("a");r.style.display="none",r.href=A,r.download=e.name,document.body.appendChild(r),r.click(),document.body.removeChild(r)}):(F.classList.add("hidden"),F.onclick=null)),e.type.startsWith("image/")){const r=document.createElement("img");r.src=A,a.appendChild(r)}else if(e.type.startsWith("video/")||e.type.startsWith("audio/")){const r=document.createElement(e.type.startsWith("video/")?"video":"audio");r.src=A,r.controls=!0,r.autoplay=!0,i||(r.setAttribute("controlsList","nodownload"),r.oncontextmenu=s=>s.preventDefault()),a.appendChild(r)}else if(e.type==="application/pdf"){const r=document.createElement("iframe");r.src=A+(i?"":"#toolbar=0"),r.style.width="100%",r.style.height="100%",a.appendChild(r)}else e.type==="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"||e.type==="application/vnd.ms-excel"||e.name.endsWith(".xlsx")||e.name.endsWith(".xls")||e.name.endsWith(".csv")?lt(n,a):e.type==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"||e.name.endsWith(".docx")?await dt(n,a):a.innerText="Preview not supported for this file type.";document.getElementById("viewer-filename").innerText=e.name,Me.classList.remove("hidden"),St(e),document.getElementById("viewer-timer").classList.add("hidden")}catch(n){console.error(n),await f("Error","Decryption failed.")}}async function pe(){Me.classList.add("hidden"),F&&F.classList.add("hidden"),document.getElementById("viewer-content").innerHTML="",A&&(URL.revokeObjectURL(A),A=null)}async function Ut(){localStorage.getItem("sv_crash_guard")&&(localStorage.removeItem("sv_crash_guard"),L())}async function L(e=""){oe.innerHTML="";let t=await E.getAllFiles();P=t,ft(t),e&&(t=t.filter(a=>a.name.toLowerCase().includes(e.toLowerCase())));const n=Pe?.value||"date-desc";t.sort((a,i)=>{if(a.favorite&&!i.favorite)return-1;if(!a.favorite&&i.favorite)return 1;switch(n){case"date-desc":return(i.date||0)-(a.date||0);case"date-asc":return(a.date||0)-(i.date||0);case"name-asc":return a.name.localeCompare(i.name);case"name-desc":return i.name.localeCompare(a.name);case"size-desc":return(i.size||0)-(a.size||0);case"size-asc":return(a.size||0)-(i.size||0);default:return 0}});const o=document.getElementById("quick-tips");if(o&&(P.length===0?o.classList.remove("hidden"):o.classList.add("hidden")),t.length===0){oe.innerHTML=`
    ...
    `,r.querySelector(".select-checkbox").onchange=p=>{p.stopPropagation(),p.target.checked?x.add(a.id):x.delete(a.id),G()},r.querySelector(".open-btn").onclick=p=>{p.stopPropagation(),de(a.id)},r.querySelector(".download-btn").onclick=p=>Pt(p,a.id),r.querySelector(".info-btn").onclick=p=>Dt(p,a.id),r.querySelector(".rename-btn").onclick=p=>Mt(p,a.id,a.name),r.querySelector(".share-btn").onclick=p=>Ot(p,a.id),r.querySelector(".custom-share-btn").onclick=p=>Nt(p,a.id),r.querySelector(".delete-btn").onclick=p=>At(p,a.id),oe.appendChild(r)})}function ue(){const e=localStorage.getItem("sv_panic_action")||"blur",t=document.getElementById("panic-action-select");t&&(t.value=e);const n=localStorage.getItem("sv_dl_media")!=="false",o=localStorage.getItem("sv_dl_doc")!=="false",a=document.getElementById("dl-media-toggle"),i=document.getElementById("dl-doc-toggle");a&&(a.checked=n),i&&(i.checked=o),le();const r=localStorage.getItem("sv_panic_enabled")!=="false",s=document.getElementById("panic-enable-toggle");s&&(s.checked=r),he()}function he(){const e=localStorage.getItem("sv_panic_enabled")!=="false",t=document.getElementById("header-panic-wrapper"),n=document.getElementById("panic-settings-content");t&&(e?(t.classList.remove("hidden"),t.style.display="flex"):(t.classList.add("hidden"),t.style.display="none")),n&&(e?(n.classList.remove("hidden"),n.style.opacity="1",n.style.pointerEvents="auto"):(n.classList.add("hidden"),n.style.opacity="0.5",n.style.pointerEvents="none"))}function me(){const e=localStorage.getItem("sv_panic_action")||"blur";if(e!=="none"){if(e==="erase")(async()=>{try{const t=localStorage.getItem("sv_panic_action"),n=await E.getAllFiles();for(const o of n)await E.deleteFile(o.id);localStorage.clear(),t&&localStorage.setItem("sv_panic_action",t),window.location.reload()}catch(t){console.error(t)}})();else if(e==="blur"||e==="lock"){pe(),z?.close(),ee?.close(),document.querySelectorAll(".panic-overlay").forEach(n=>n.remove());const t=document.createElement("div");t.className="panic-overlay blur-mode",t.innerHTML=`
    ...
    `,document.body.appendChild(t),document.getElementById("panic-reload-btn")?.addEventListener("click",n=>{n.stopPropagation(),t.remove()})}else if(e==="loading"){pe(),z?.close(),ee?.close(),document.querySelectorAll(".panic-overlay").forEach(o=>o.remove());const t=document.createElement("div");t.className="panic-overlay",t.innerHTML=`
